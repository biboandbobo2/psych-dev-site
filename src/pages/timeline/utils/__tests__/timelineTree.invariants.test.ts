...
describe('I2+I8: совпадение x разных ветвей (ограничение ТОЛЬКО для legacy-узлов без branchId)', () => {
  // Для узлов БЕЗ branchId принадлежность кодируется координатой
  // node.parentX === edge.x, и при shared-x она принципиально
  // неразличима: дерево детерминированно отдаёт события первой
  // встреченной ветке (защита от дублей — I1).
  // Фаза 2 (RFC timeline-branch-id) закрывает Д5 для данных со ссылкой —
  // см. timelineTree.branchId.test.ts. В живом приложении данные всегда
  // проходят normalizeImportedTimelineData, которая досеивает branchId,
  // поэтому it.fails ниже описывают только СЫРЫЕ (не нормализованные)
  // legacy-графы. Если тест внезапно пройдёт — поведение fallback'а
  // изменилось, пересмотреть RFC.
