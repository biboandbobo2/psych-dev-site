...
export function LoadState({ error, retry }: { error?: string; retry?: () => void }) {
  return (
    <div className="ico-empty" role="status">
      <p>{error || 'Открываем коллекцию…'}</p>
      {error && <button className="ico-button" onClick={retry}>Попробовать ещё раз</button>}
    </div>
  );
}
...
export function PassportFacts({ icon }: { icon: IconRecord }) {
  ...
  const facts: [string, string][] = [
    ['Сюжет', icon.subject],
    ['Персонажи', icon.people],
    ['Иконографический тип', icon.type],
    ['Традиция', icon.tradition],
    ['Регион / школа', icon.region],
    ['Период', icon.period],
    ['Автор / мастерская / круг', icon.attribution],
    ['Собрание', icon.museum],
    ['Инвентарный номер', icon.inventory],
    ['Материал', icon.material],
  ];
  // Строки-заглушки («Не указан в доступном источнике») ничего не сообщают — их не показываем.
  const shown = facts.filter(([, value]) => !isPlaceholderFact(value));
  return (
    <dl className="ico-facts">
      {shown.map(([term, value]) => (
        <div key={term}>
          <dt>{term}</dt>
          <dd>
            {term === 'Регион / школа' && icon.schoolId ? (
              <Link
                className="ico-link"
                to={`/iconography/schools/${icon.schoolId}`}
                state={{ returnTo: `${location.pathname}${location.search}`, returnLabel: 'Назад к иконе', returnState: location.state }}
              >{value}</Link>
            ) : value}
          </dd>
        </div>
...
export function Sources({ icon }: { icon: IconRecord }) {
  const original = {
    'Название': icon.originalMetadata.title,
    'Дата': icon.originalMetadata.date,
    'Культура': icon.originalMetadata.culture,
    'Атрибуция': icon.originalMetadata.attribution,
    'Материал': icon.originalMetadata.material,
  };
  return (
    <section className="ico-sources">
      <h2>Источники и изображение</h2>
      <ul>
...
export function Passport() {
  ...
  return (
    <>
      <Helmet>
        <title>{icon.title} — Иконография</title>
        <meta name="description" content={`${icon.title}. ${period}. ${icon.description}`} />
      </Helmet>
      <section className="ico-section">
        <BackLink />
        <div className="ico-passport">
          <div className="ico-passport-art">
            <Artwork icon={icon} priority zoom />
            <p className="ico-small">{icon.rights.credit} · {icon.rights.label}</p>
          </div>
          <div>
        ...
        <div className="ico-reading">
          ...
          {icon.reading && (
            <section className="ico-reading-links">
              <h2>Почитать о персонаже и сюжете</h2>
              <ul>
                {icon.reading.map((link) => (
                  <li key={link.url}>
                    <a className="ico-link" href={link.url} target="_blank" rel="noreferrer">{link.label}</a>
                  </li>
                ))}
              </ul>
            </section>
          ...
          {icon.schoolId && (
            <Link
              className="ico-link"
              to={`/iconography/schools/${icon.schoolId}`}
              state={{ returnTo: `/iconography/icon/${icon.id}`, returnLabel: 'Назад к иконе', returnState: location.state }}
            >О школе и художественной традиции</Link>
          )}

          <Sources icon={icon} />
          <BackLink />
          <Feedback iconId={icon.id} />
        </div>
      </section>
    </>
  );
