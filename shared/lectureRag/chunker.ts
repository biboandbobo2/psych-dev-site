...
import {
  LECTURE_RAG_VERSION,
  type LectureRagChunk,
  type LectureRagChunkDocShape,
  type LectureRagSourceDocShape,
} from "./schema.js";

const LECTURE_RAG_CHUNK_MAX_SEGMENTS = 6;
const LECTURE_RAG_CHUNK_MAX_CHARS = 1200;

function normalizeLectureRagText(text: string) {
  return text.toLowerCase().replace(/\s+/g, " ").trim();
}

function buildChunkText(parts: string[]) {
  return parts
...
function getTranscriptDurationMs(transcript: TranscriptImportResult) {
  const lastSegment = transcript.segments[transcript.segments.length - 1];
  return lastSegment?.endMs ?? null;
}

function finalizeLectureChunk(
  entries: TranscriptImportResult["segments"],
  chunkIndex: number
): LectureRagChunk | null {
  if (!entries.length) {
    return null;
  }
...
export function buildLectureRagChunks(transcript: TranscriptImportResult) {
  const chunks: LectureRagChunk[] = [];
  let currentSegments: TranscriptImportResult["segments"] = [];
  ...
  transcript.segments.forEach((segment) => {
    ...
    const shouldFlush =
      currentSegments.length >= LECTURE_RAG_CHUNK_MAX_SEGMENTS ||
...
export function buildLectureKey(reference: VideoReference, youtubeVideoId: string) {
  return `${reference.courseId}::${reference.lessonId}::${youtubeVideoId}`;
}

export function buildLectureRagSourceDoc<TTimestamp>(
  reference: VideoReference,
  youtubeVideoId: string,
  transcript: TranscriptImportResult,
  chunkCount: number,
  now: TTimestamp
): LectureRagSourceDocShape<TTimestamp> {
  return {
...
export function buildLectureRagChunkDocs<TTimestamp, TVector>(
  reference: VideoReference,
  youtubeVideoId: string,
  chunks: LectureRagChunk[],
  embeddings: TVector[],
  now: TTimestamp
) {
  if (chunks.length !== embeddings.length) {
    throw new Error(
      `Chunks/embeddings mismatch for ${reference.title}: ${chunks.length} chunks vs ${embeddings.length} embeddings`
    );
  }
  ...
  return chunks.map((chunk, index) => ({
    ...
    data: {
      ...
      updatedAt: now,
      version: LECTURE_RAG_VERSION,
    } satisfies LectureRagChunkDocShape<TTimestamp, TVector>,
  }));
}
