...
import type { TranscriptImportTarget } from "./importTypes.js";

export function getVideoEntriesFromContentDoc(
  data: Record<string, unknown>,
  fallbackTitle: string
) {
  const candidates: Array<{ title: string; url: string }> = [];
  const sections = (data.sections as Record<string, any> | undefined) ?? {};
...
export function registerTranscriptTarget(
  targets: Map<string, TranscriptImportTarget>,
  docPath: string,
  courseId: string,
  lessonId: string,
  data: Record<string, unknown>
