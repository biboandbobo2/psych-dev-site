...
- Используйте `/admin/content` (доступно для администраторов) — интерфейс редактирует документы в коллекциях Firestore `periods` и `intro`.
- Снимайте метку `published`, Accent и прочие поля прямо в форме, изменения сразу начинают отдавать фронтенду.
- Если нужно, можно корректировать поля вручную через Firebase Console (`periods/{periodId}`, `intro/singleton`), но после этого достаточно обновить страницу или перезапустить dev-сервер.
