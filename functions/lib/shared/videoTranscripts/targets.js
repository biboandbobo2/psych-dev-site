...
export function getVideoEntriesFromContentDoc(data, fallbackTitle) {
    const candidates = [];
    ...
    if (!candidates.length && Array.isArray(data.video_playlist)) {
        data.video_playlist.forEach((entry) => {
            const url = typeof entry?.url === "string" ? entry.url.trim() : "";
            if (!url) {
                return;
            }
...
export function registerTranscriptTarget(targets, docPath, courseId, lessonId, data) {
    const fallbackTitle = (typeof data.title === "string" && data.title.trim()) ||
        (typeof data.label === "string" && data.label.trim()) ||
        lessonId;
    getVideoEntriesFromContentDoc(data, fallbackTitle).forEach((video) => {
...
export async function collectTranscriptTargets(db) {
    const targets = new Map();
    const scanCollection = async (collectionName, courseId) => {
        const snapshot = await db.collection(collectionName).get();
        snapshot.forEach((docSnap) => {
            registerTranscriptTarget(targets, `${collectionName}/${docSnap.id}`, courseId, docSnap.id, docSnap.data());
        });
    };
    ...
    if (introSingleton.exists) {
        registerTranscriptTarget(targets, "intro/singleton", "development", "intro", introSingleton.data() ?? {});
    }
    ...
    for (const courseSnap of coursesSnapshot.docs) {
        ...
        lessonsSnapshot.forEach((lessonSnap) => {
            registerTranscriptTarget(targets, `courses/${courseSnap.id}/lessons/${lessonSnap.id}`, courseSnap.id, lessonSnap.id, lessonSnap.data());
        });
    }
    return [...targets.values()].sort((a, b) => a.youtubeVideoId.localeCompare(b.youtubeVideoId));
}
export function buildManualTranscriptTarget(video) {
