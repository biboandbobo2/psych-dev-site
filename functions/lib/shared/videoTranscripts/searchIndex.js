...
function finalizeSearchChunk(segments, chunkIndex) {
    if (!segments.length) {
        return null;
    }
...
function shouldStartNewChunk(currentSegments, nextSegment) {
...
export function buildTranscriptSearchChunks(segments) {
    const chunks = [];
    let currentSegments = [];
...
export function buildTranscriptSearchReferenceKey(reference, referenceIndex) {
    return `${reference.courseId}::${reference.lessonId}::${referenceIndex}`;
}
export function buildTranscriptSearchParentDoc(target, chunkCount, now) {
    return {
...
export function buildTranscriptSearchChunkDocs(target, transcript, now) {
    const chunks = buildTranscriptSearchChunks(transcript.segments);
    const parentDoc = buildTranscriptSearchParentDoc(target, chunks.length * target.references.length, now);
