...
async function updateLectureJob(jobId, updates) {
    ...
    if (updates.chunkCount !== undefined)
        payload.chunkCount = updates.chunkCount;
    if (updates.error !== undefined)
        payload.error = updates.error;
    if (updates.finishedAt !== undefined)
        payload.finishedAt = updates.finishedAt;
    if (updates.lectureKeys !== undefined)
        payload.lectureKeys = updates.lectureKeys;
    if (updates.progress !== undefined)
        payload.progress = updates.progress;
    if (updates.referenceCount !== undefined)
        payload.referenceCount = updates.referenceCount;
    if (updates.status !== undefined)
        payload.status = updates.status;
    if (updates.step !== undefined)
        payload.step = updates.step;
    if (updates.youtubeVideoId !== undefined)
        payload.youtubeVideoId = updates.youtubeVideoId;
    if (updates.log) {
        payload.logs = FieldValue.arrayUnion(updates.log);
    }
    await jobRef.set(payload, { merge: true });
}
async function ensureLectureJob(jobId, youtubeVideoId) {
    const db = getFirestore();
    const jobRef = db.collection(LECTURE_RAG_JOBS_COLLECTION).doc(jobId);
...
async function resolveTranscriptTarget(youtubeVideoId) {
    const db = getFirestore();
    const targets = await collectTranscriptTargets(db);
    return (targets.find((target) => target.youtubeVideoId === youtubeVideoId) ?? null);
}
function buildResponseSummary(target, jobId, result) {
    return {
