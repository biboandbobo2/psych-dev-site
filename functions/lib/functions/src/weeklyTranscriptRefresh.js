...
export async function runWeeklyTranscriptRefresh(deps) {
    ...
    const lock = await acquireTranscriptRefreshLock(db, config.jobName, "scheduled", startedAt, config.lockTtlMinutes);
    if (!lock.acquired) {
    ...
    try {
        const candidateResult = await collectTranscriptRefreshCandidates(db, startedAt, config.batchSize);
        ...
        for (const candidate of candidateResult.candidates) {
            const result = await upsertTranscript({ db, bucket }, candidate.target, {
                dryRun: false,
                force: false,
                langs: config.langs,
            });
            ...
            summary.errorSummary.push(`${candidate.target.youtubeVideoId}: ${result.errorCode ?? "UNKNOWN"}`);
        }
        const finishedAt = Timestamp.now();
        await finalizeTranscriptRefreshRun(lock, finishedAt, summary, "completed", null);
        await sendMessage(formatTranscriptRefreshTelegramReport(summary, {
