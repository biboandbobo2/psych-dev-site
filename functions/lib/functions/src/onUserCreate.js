...
export const onUserCreate = functions.auth.user().onCreate(async (user) => {
    ...
    try {
        ...
        if (email) {
            try {
                ...
                if (memberSnap.size + announcerSnap.size > 0) {
                    debugLog(`🔁 Replaced pendingUid in ${memberSnap.size + announcerSnap.size} group(s) for ${email}`);
                }
            }
            catch (err) {
        ...
        try {
            ...
            debugError(`⚠️ Failed to add ${uid} to '${EVERYONE_GROUP_ID}' group (run scripts/ensure-everyone-group.ts):`, err);
        }
