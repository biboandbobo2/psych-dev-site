...
function assertSuperAdmin(context) {
    ...
    if (callerEmail !== SUPER_ADMIN_EMAIL) {
        throw new functions.https.HttpsError("permission-denied", "Только super-admin может управлять администраторами");
    }
    return context.auth.uid;
}
function normalizeEditableCourses(raw) {
    if (!Array.isArray(raw))
        return [];
    return Array.from(new Set(raw.filter((c) => typeof c === "string" && c.trim().length > 0)));
}
/**
...
export const makeUserAdmin = functions.https.onCall(async (data, context) => {
    ...
    const editableCourses = normalizeEditableCourses(data.editableCourses);
    if (editableCourses.length === 0) {
        throw new functions.https.HttpsError("invalid-argument", "Укажите хотя бы один курс, который админ сможет редактировать");
    }
...
export const removeAdmin = functions.https.onCall(async (data, context) => {
    ...
    const existingClaims = userRecord.customClaims ?? {};
    const { role: _r, editableCourses: _ec, ...rest } = existingClaims;
    void _r;
    void _ec;
...
export const setAdminEditableCourses = functions.https.onCall(async (data, context) => {
    ...
    if (role !== "admin") {
        throw new functions.https.HttpsError("failed-precondition", "Editable courses можно задать только пользователю с ролью admin");
    }
