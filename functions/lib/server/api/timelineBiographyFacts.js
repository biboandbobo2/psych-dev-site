import { inferBirthDetailsFromExtract, inferDeathYearFromExtract, normalizeFactText, normalizeSphere, normalizeText, } from './timelineBiographyHeuristics.js';
...
function parseTimePrecision(value) {
...
function inferEventTypeFromSphere(sphere) {
    switch (sphere) {
        case 'education':
            return 'education';
        case 'career':
...
function normalizeFactSphereValue(sphere, eventType) {
    if (eventType === 'birth')
        return 'family';
    if (eventType === 'death')
        return 'health';
    if (eventType === 'move')
        return 'place';
    if (eventType === 'publication' || eventType === 'project')
        return 'creativity';
    if (eventType === 'friends')
        return 'friends';
    if (eventType === 'education')
        return 'education';
    if (eventType === 'family')
        return 'family';
    if (eventType === 'health')
        return 'health';
    if (eventType === 'career' || eventType === 'award' || eventType === 'conflict')
        return sphere ?? 'career';
    return sphere;
}
...
function isRegistryMetadataEvidence(value) {
...
function scoreFactCandidateForDedupe(candidate) {
    ...
    const precisionScore = candidate.timePrecision === 'exact'
        ? 3
...
function isBrokenFactFragment(value) {
    if (!value)
        return false;
    const normalized = value.trim();
    if (normalized.length < 4)
        return true;
    if (/^(?:[А-ЯA-Z]\.){1,4}$/u.test(normalized))
        return true;
    if (/^\d{4}[^А-Яа-яA-Za-zЁё]*[А-ЯA-Z]\.$/u.test(normalized))
        return true;
    return false;
}
...
export function buildFactCandidateKey(candidate) {
    const ageOrYear = Number.isFinite(candidate.age) ? `age:${candidate.age}` : Number.isFinite(candidate.year) ? `year:${candidate.year}` : 'unknown';
...
export function normalizeFactCandidate(candidate) {
    ...
    const timePrecision = parseTimePrecision(candidate.timePrecision) ??
    ...
    if (!labelHint && !evidence)
        return null;
    if (!labelHint && isBrokenFactFragment(evidence))
        return null;
    if (!Number.isFinite(inferredAge) && !Number.isFinite(year))
        return null;
    if (Number.isFinite(inferredAge) && Number(inferredAge) === 0 && eventType !== 'birth')
        return null;
    const normalized = {
        year,
        age: inferredAge,
...
export function dedupeFactCandidates(candidates) {
...
export function normalizeFactCandidates(candidates) {
    return dedupeFactCandidates(candidates
        .map((candidate) => normalizeFactCandidate(candidate))
        .filter((candidate) => Boolean(candidate)));
}
export function parseLineBasedBiographyFactCandidates(rawText) {
    const facts = [];
    const lines = rawText
        .split(/\r?\n/)
        .map((line) => line.trim())
...
function hasFactInAgeWindow(facts, minAge, maxAge) {
    return facts.some((fact) => Number.isFinite(fact.age) && Number(fact.age) >= minAge && Number(fact.age) <= maxAge);
}
function buildFactSemanticTokens(candidate) {
    return new Set(normalizeFactText(candidate.evidence || candidate.labelHint)
        .split(/\s+/)
        .map((token) => token.trim())
        .filter((token) => token.length >= 5));
}
function factsShareApproximateTime(a, b) {
...
function hasEquivalentFact(facts, candidate) {
...
function selectBestFactForWindow(candidates, minAge, maxAge) {
...
export function ensureEarlyLifeFactCoverage(facts, heuristicFacts, currentAge) {
    if (currentAge < 12)
        return facts;
    const merged = [...facts];
    const windows = [
    ...
    for (const [minAge, maxAge] of windows) {
        if (hasFactInAgeWindow(merged, minAge, maxAge))
            continue;
