import { BRANCH_SLOT_ORDER, BRANCH_X_OFFSETS, DEFAULT_BRANCH_LENGTH, EVENT_ICON_IDS, SPHERE_META, TIMELINE_PERIODIZATION_IDS, LINE_X_POSITION, } from './timelineBiographyTypes.js';
import { isGenericBiographyLabel, isTruncatedBiographyLabel } from './timelineBiographyLabelQuality.js';
export function normalizeSphere(sphere) {
    return typeof sphere === 'string' && sphere in SPHERE_META ? sphere : undefined;
}
export function normalizeIcon(iconId) {
    return typeof iconId === 'string' && EVENT_ICON_IDS.includes(iconId)
        ? iconId
        : undefined;
}
...
export function russianDateToISO(dateStr) {
    if (!dateStr)
        return undefined;
...
export function pickBranchX(sphere, startAge, endAge, occupied) {
...
export function extractQuotedWorkTitle(value) {
    const title = value.match(/[«"](.*?)[»"]/u)?.[1]?.trim();
    if (!title)
        return undefined;
    if (!/^[A-ZА-ЯЁ0-9]/u.test(title))
        return undefined;
    return title;
}
...
export function buildEventFactKey(event) {
...
export function hasTerminalLifeEvent(events, currentAge) {
    const terminalEvent = events.find((event) => /(смерт|гибел|погиб|умер|дуэл|died|death|killed)/i.test(`${event.label} ${event.notes ?? ''}`));
    if (!terminalEvent)
        return false;
    return Math.abs(terminalEvent.age - currentAge) <= 1.5;
}
export function splitBiographyExtractIntoSentences(extract) {
    return extract
        .split(/(?<=[.!?])\s+|\n+/)
...
// ---------------------------------------------------------------------------
function extractYears(sentence) {
...
function isMediaContextSentence(sentence) {
...
function normalizeLossRelationLabel(value) {
    ...
    if (normalized.startsWith('муж'))
        return 'мужа';
    if (normalized.startsWith('сын'))
        return 'сына';
    if (normalized.startsWith('дочер'))
        return 'дочери';
    if (normalized.startsWith('брат'))
        return 'брата';
    if (normalized.startsWith('сестр'))
        return 'сестры';
    if (normalized.startsWith('друг'))
        return 'друга';
    if (normalized.startsWith('нян'))
        return 'няни';
    if (normalized.startsWith('опекун'))
        return 'опекуна';
    return normalized;
}
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
export function inferBirthDetailsFromExtract(extract) {
    ...
    const leadDateRangeMatch = extractHead.match(/(\d{1,2}\s+[A-Za-zА-Яа-яЁё]+\s+\d{4})\s*,\s*([^—()]+?)\s*[—-]\s*(\d{1,2}\s+[A-Za-zА-Яа-яЁё]+\s+\d{4})/u);
    ...
    const placeMatch = birthSentence.match(/родил[а-яёa-z]*\s+(?:в|in)\s+([^,.();]+?)(?:\s+(?:в|in)\s+\d{4}\b|[,.();]|$)/i) ??
        birthSentence.match(/\(\s*[^,]+,\s*([^—,)]+?)(?:\s*[—,)])/);
    const normalizedPlace = normalizeLocationCandidate(placeMatch?.[1]);
    const place = (leadBirthPlace && isLikelyBirthPlace(leadBirthPlace) ? leadBirthPlace : undefined) ??
...
export function inferDeathYearFromExtract(extract) {
    const extractHead = extract.slice(0, 600);
    ...
    const deathSentence = sentences.find((sentence) => {
        ...
        if (/(отца|отец|матери|мать|родителей|родителя|мужа|муж|жены|жена|сына|сын|дочери|дочь|сестры|сестра|брата|брат|друга|друг|опекун)/i.test(sentence) &&
...
// ---------------------------------------------------------------------------
export function inferIconFromSentence(sentence, sphere) {
    if (isRegistryMetadataSentence(sentence))
        return 'baby-feet';
    if (/родил|born/i.test(sentence))
        return 'baby-feet';
    if (/женил|брак|свад/i.test(sentence))
        return 'wedding-rings';
    if (/лице|школ|универс|академ|учил|нян|настав/i.test(sentence))
        return 'school-backpack';
    if (/опублик|издал|поэм|роман|стих|книг|произвед|написал|сочин/i.test(sentence))
        return 'idea-book';
    if (/ссыл|переех|эмигр|travel|moved/i.test(sentence))
        return 'passport';
    if (/умер|погиб|дуэл|болез/i.test(sentence))
        return 'thermometer';
    if (/друж|переписк|кружок|декабрист/i.test(sentence))
        return 'friendship';
    if (/ребён|сын|дочь/i.test(sentence))
        return 'baby-stroller';
...
export function inferSphereFromSentence(sentence) {
    if (/родил[а-яё]?\b|рожден/i.test(sentence) && sentence.length < 80)
        return 'family';
    if (/свад|женил|брак|брач|замуж|обручен/i.test(sentence))
        return 'family';
    if (/ребён|сын|дочь|внук|внучк/i.test(sentence))
        return 'family';
    if (/школ|университ|лицей|семинар|обучен|учил|курс|академ.*(?:поступ|учил|оконч)|диплом|диссертац|студен/i.test(sentence))
        return 'education';
    if (/опублик|поэм|роман|стих|книг|произвед|повесть|пьес|журнал|газет|издан|написал|сочин|поэзи|литератур/i.test(sentence))
        return 'creativity';
    if (/переех|ссыл|эмигр|поездк|travel|move|прибыл|отправ|уехал|вернул(?:ся|ась)/i.test(sentence))
        return 'place';
    if (/болез|здоров|лечен|ранен|операц|clinic|hospital|травм/i.test(sentence))
        return 'health';
    if (/умер|скончал|погиб|смерт|похорон|гибел|died|death/i.test(sentence))
        return 'health';
    if (/друг|друж|переписк|знакомств|встреч/i.test(sentence))
        return 'friends';
    if (/арест|тюрьм|ссыл|заключ|конфликт|суд|дуэл|дисципл/i.test(sentence))
        return 'health';
    if (/назначен|служб|карьер|начальник|директор|глав|министр|чиновник|секретар|должнос/i.test(sentence))
        return 'career';
    if (/наград|орден|медал|звани|прем|лауреат|почётн|academ|fellow|member/i.test(sentence))
        return 'career';
    return 'other';
}
export function buildHeuristicLabel(sentence, sphere) {
    const workTitle = extractQuotedWorkTitle(sentence);
    const outletTitle = extractPublicationOutletTitle(sentence);
    const location = normalizeLocationCandidate(sentence.match(/\b(?:в|на|из)\s+([А-ЯЁA-Z][^,.();:]{2,50})/u)?.[1]?.trim().replace(/\s+/g, ' ') ?? undefined);
    ...
    if (/родил|born/i.test(sentence))
        return 'Рождение';
    if (/дуэл/i.test(sentence) && /умер|погиб|скончал|died/i.test(sentence))
        return 'Дуэль и смерть';
    ...
    if (/умер|скончал|погиб|died/i.test(sentence))
        return 'Смерть';
    if (/женил|брак|свад/i.test(sentence))
        return spouse ? `Брак с ${spouse}` : 'Брак';
    if (/родил\S*\s+(?:сын|дочь|ребён)/i.test(sentence))
        return 'Рождение ребёнка';
    if (/нян|домашн.*обуч|гуверн|наставник/i.test(sentence))
        return 'Домашнее обучение';
    if (/перв.*стих/i.test(sentence))
        return 'Первые стихи';
    if (/ранн.*чтен|много читал|библиотек/i.test(sentence))
        return 'Раннее увлечение чтением';
    if (/(детств|летние месяцы|усадьб|у бабушк|у дедушк)/i.test(sentence) && location)
        return `Детство в ${location}`;
    if (/под надзор/i.test(sentence))
        return 'Полицейский надзор';
    if (/путешеств|поездк|поехал|отправился/i.test(sentence) && location)
        return `Поездка в ${location}`;
    if (/вернул(?:ся|ась|ись)?\s+из\s+ссыл/i.test(sentence))
        return 'Возвращение из ссылки';
    if (/южн/i.test(sentence) && /ссыл/i.test(sentence))
        return 'Южная ссылка';
    if (/сослан|ссыл/i.test(sentence))
        return location ? `Ссылка в ${location}` : 'Ссылка';
    if (/переех|эмигр|relocat|moved/i.test(sentence))
        return location ? `Переезд в ${location}` : 'Переезд';
    if (/поступ/i.test(sentence) && institution && sphere === 'education')
        return `Поступление в ${institution}`;
    if (/(окончил|выпустил|выпуск)/i.test(sentence) && institution)
        return `Окончание ${institution}`;
    if (/лице|школ|универс|академ|институт|учил/i.test(sentence) && institution && sphere === 'education') {
        return `Учёба в ${institution}`;
    }
    if (/лицей/i.test(sentence) && sphere === 'education')
        return 'Лицейские годы';
    if (/университет|академ|институт/i.test(sentence) && sphere === 'education')
        return 'Студенческие годы';
    if (/школ/i.test(sentence) && sphere === 'education')
        return 'Школьные годы';
    if (/вступил в .*общество/i.test(sentence))
        return 'Литературный круг';
    if (outletTitle && /газет|журнал/i.test(sentence))
        return `Материал в «${outletTitle}»`;
    if (workTitle && isMediaContextSentence(sentence) && !isSubjectAuthorshipSentence(sentence)) {
        return `Упоминание «${workTitle}»`;
    }
    if (workTitle && (sphere === 'career' || sphere === 'creativity'))
        return `Публикация «${workTitle}»`;
    if (/начал работу/i.test(sentence) && workTitle)
        return `Начало работы над «${workTitle}»`;
    if (/опублик|издал|поэм|роман|стих|книг|произведени|published/i.test(sentence)) {
        if (/впервые|дебют|получил известность|выступил/i.test(sentence))
            return 'Первое литературное признание';
        if (location)
            return `Публикация в ${location}`;
    }
    if (/поступил на службу|начал службу/i.test(sentence))
        return 'Начало службы';
    if (/долг|обязательств|финанс|денег|money|finance/i.test(sentence))
        return 'Финансовое давление';
    if (/арест|тюрьм|prison|arrest/i.test(sentence))
        return 'Арест';
    if (/цензур|запрет|censor/i.test(sentence))
        return 'Цензурные ограничения';
    if (/наград|орден|award|prize/i.test(sentence))
        return 'Награждение';
    ...
    if (words.length > 7) {
        ...
        if (shortPhrase.length <= 50)
            return shortPhrase;
    }
    return cleaned.length > 50 ? `${cleaned.slice(0, 47).trimEnd()}...` : cleaned;
}
export function isRawSentenceLabel(label) {
    if (isGenericBiographyLabel(label) || isTruncatedBiographyLabel(label))
        return true;
    if (label.length > 50)
        return true;
    if (/^\d{1,2}\s+[а-яё]+\s+\d{4}/u.test(label))
        return true;
    if (/^(?:В|С|К|После|Весной|Летом|Осенью|Зимой)\s+\d{4}/u.test(label))
        return true;
    if (/^(?:С|В)\s+\d{1,2}\s+[а-яё]+\s+\d{4}/u.test(label))
        return true;
    if (/^(?:Однако|Таким образом|В своей|В своём|После знакомства|Из-за|из-за|Тогда|Но|При этом)\b/u.test(label))
        return true;
    if (/^(?:В|С|После|Из-за|Тогда|Но)\s+[а-яё]/u.test(label) && label.split(/\s+/).length >= 4)
        return true;
    if (/^(?:один|два|три|четыре|пять|шесть|семь|восемь|девять|десять|несколько|многие)\b/iu.test(label))
        return true;
    if (/^[А-ЯЁ][а-яё]+\s+[а-яё]+\s+[а-яё]+\s+[а-яё]+\s+[а-яё]+\s+[а-яё]+\s+[а-яё]+/u.test(label))
        return true;
    if (/^[а-яё]/u.test(label))
        return true;
    if (/\s(?:в|на|с|о|к|у|по|из|за|от|до|для|при|про|без|над|под|об|что|как|и|а|но|или|не)\s*$/i.test(label))
        return true;
    if (/\b(?:он|она|его|её|они|их|ему|ей|им)\b/i.test(label))
        return true;
    if (label.split(/\s+/).length >= 8)
        return true;
    return false;
}
function postProcessModelEvent(event) {
...
export function sanitizeTimelineEventPlan(event, fallbackSphere) {
    if (!Number.isFinite(event.age))
        return null;
...
export { DEFAULT_BRANCH_LENGTH, LINE_X_POSITION, TIMELINE_PERIODIZATION_IDS, };
