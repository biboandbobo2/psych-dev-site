import { inferIconFromSentence, normalizeText, } from './timelineBiographyHeuristics.js';
import { SPHERE_META, } from './timelineBiographyTypes.js';
// ---------------------------------------------------------------------------
// Sphere mapping: composition returns free-form sphere strings,
// BiographyTimelinePlan uses the TimelineSphere enum.
// ---------------------------------------------------------------------------
const SPHERE_MAPPING = {
    politics: 'career',
    conflict: 'health',
    legacy: 'other',
    exile: 'place',
...
function mapSphere(sphere) {
    if (sphere in SPHERE_META)
        return sphere;
...
// ---------------------------------------------------------------------------
function pickIcon(fact) {
    const cat = fact.category ?? fact.eventType ?? '';
    if (cat === 'birth')
        return 'baby-feet';
    if (cat === 'death')
        return 'thermometer';
    if (cat === 'family' && /бра[кч]|свадьб|женил|вышла замуж/i.test(fact.details))
        return 'wedding-rings';
    if (cat === 'family' && /ребён|сын|дочь|родил/i.test(fact.details))
    ...
    if (cat === 'award')
        return 'trophy';
    if (cat === 'conflict' && /арест/i.test(fact.details))
        return 'id-card';
...
// Label truncation
// ---------------------------------------------------------------------------
function truncateLabel(text, maxLen = 35) {
...
// Year → age conversion
// ---------------------------------------------------------------------------
function yearToAge(year, birthYear, lifespan) {
...
// ---------------------------------------------------------------------------
function factToEventPlan(fact, birthYear, lifespan) {
    if (fact.year == null)
        return null;
    ...
    return {
        ...
        iconId: pickIcon(fact),
    };
}
// ---------------------------------------------------------------------------
// Birth detection helpers
// ---------------------------------------------------------------------------
function findBirthFact(facts) {
    return facts.find(f => f.eventType === 'birth' || f.category === 'birth');
}
function findDeathFact(facts, birthYear) {
    ...
    const deathCandidates = facts.filter(f => (f.eventType === 'death' || f.category === 'death') &&
    ...
    return pool.reduce((latest, f) => (f.year ?? 0) > (latest.year ?? 0) ? f : latest);
}
// ---------------------------------------------------------------------------
// Build birth details from fact
// ---------------------------------------------------------------------------
function buildBirthDetails(birthFact) {
    const details = {};
    if (birthFact) {
...
// ---------------------------------------------------------------------------
function mergeSameAgeEvents(events) {
    const byAge = new Map();
...
export function buildPlanFromCompositionResult(params) {
    ...
    if (legacyFacts.length > 0) {
        ...
        const legacyEvents = legacyFacts.map((f, i) => ({
