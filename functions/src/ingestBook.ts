...
import { chunkPages, DEFAULT_CHUNK_CONFIG, type TextChunk } from './lib/chunker.js';
import { getEmbeddingsBatch, EMBEDDING_DIMS } from './lib/embeddings.js';
...
type IngestionStep = 'download' | 'extract' | 'chunk' | 'embed' | 'save';
...
async function updateJob(
  ...
  if (updates.progress) data.progress = updates.progress;
...
export const ingestBook = onRequest(
  ...
  async (req, res) => {
    ...
    const { bookId, jobId } = req.body as IngestRequest;
    ...
    try {
      ...
      if (isProbablyScan(parsed.pages)) {
        await updateJob(jobId, {
          log: 'Warning: Low text density, might be a scanned PDF',
        });
      }
      ...
      await updateJob(jobId, {
        log: `Extracted text from ${parsed.totalPages} pages`,
      });
      ...
      const embeddings = await getEmbeddingsBatch(texts, (done: number, total: number) => {
