...
export function buildBiographyUrlContextFactExtractionPrompt(params: {
  ...
  return `Ты извлекаешь биографические факты из одной статьи по URL с помощью URL context.

ВАЖНО
- Используй только содержимое страницы по этой ссылке. Не добавляй знания извне, даже если они тебе известны.
