...
const FIELDS = ['id', 'title', 'tradition', 'region', 'period', 'centuries', 'subject', 'people', 'type', 'museum', 'tags', 'image', 'recognitionGroup', 'schoolId', 'topics'];
// Вычисляемое поле: темы всех вопросов паспорта и трёх вопросов викторины.
// Практика отбирает по нему иконы, не читая паспорта.
const LEVELS = ['beginner', 'explorer', 'expert'];
const topicsOf = (r) => [...new Set([...(r.questions || []), ...LEVELS.map((level) => r.recognition && r.recognition[level])]
  .filter(Boolean).map((q) => q.topic))].sort();
...
const next = records.map((r) => {
  const source = { ...r, topics: topicsOf(r) };
  return Object.fromEntries(FIELDS.filter((k) => source[k] !== undefined).map((k) => [k, source[k]]));
});
