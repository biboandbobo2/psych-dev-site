...
TS_DATA_URL_OUTPUT = ROOT / "src" / "data" / "eventIconDataUrls.ts"
...
def write_metadata() -> None:
    ...
    for icon_id, title in ICON_ENTRIES:
        ...
        if not file_path.exists():
            raise SystemExit(f"Missing icon file: {filename}")
        ...
        json_meta.append(core_entry)
        ts_core_meta.append(core_entry)

        data_url_map[icon_id] = (
            "data:image/png;base64,"
            + base64.b64encode(file_path.read_bytes()).decode("ascii")
        )

    JSON_OUTPUT.write_text(json.dumps(json_meta, ensure_ascii=False, indent=2), encoding="utf-8")

    ts_lines = [
        "// This file is auto-generated by scripts/generate_event_icons.py",
        "// Do not edit manually.",
        "",
        "export const EVENT_ICONS = " + json.dumps(ts_core_meta, ensure_ascii=False, indent=2) + " as const;",
        "",
    ...
    data_map_lines = [
        ...
        "// Do not edit manually.",
        "",
        "import type { EventIconId } from './eventIcons';",
        "",
        "export const EVENT_ICON_DATA_URL_MAP: Record<EventIconId, string> = "
        + json.dumps(data_url_map, ensure_ascii=False, indent=2)
        + " as const;",
        "",
    ]

    TS_DATA_URL_OUTPUT.write_text("\n".join(data_map_lines), encoding="utf-8")
