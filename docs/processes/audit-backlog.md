...
| LS-3 | M (M) | Nav-индекс: 1 маленький док вместо 3 полных коллекций | Payload навигации ≤10 КБ; первый визит −0,5–1 с. **Дополнение 2026-08-23:** шторка «Список занятий» на /home (`CourseLessonsDrawer` → `useCourseNavItems`) — отдельный загрузчик: `getDocs` без таймаута, зависший промис застревает в in-flight кэше `LESSON_REQUESTS` навсегда (наблюдалось вживую: вечная «Загрузка занятий…» до перезагрузки страницы). Core-курсы перевести на SWR-хуки LS-1 (`courseContentCache` даёт мгновенный рендер из localStorage) или на nav-индекс; динамическим курсам — таймаут/ретрай |
