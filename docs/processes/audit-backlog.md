...
|----|----------|-------|-----------------------|
| ICO-1 | M (M) | Иконография: production и рост | Опубликовано 2026-09-11 в составе Академии (`/iconography`, без ссылок с сайта); итерация «до демо» 2026-09-11: рандомизация, разбор ошибок (`rationale`), итог со счётом, все 83 иконы в викторине, контент паспортов переписан по `docs/guides/iconography-quiz-design.md`, гейт `check:iconography` в `validate`. Сводный список после двух аудитов — `docs/processes/iconography-backlog.md` (блокер: темы практики не совпадают с вопросами; далее рамка признака по уровню, повторы икон, словарь сюжетов, глоссарий, расширение пула ×2 после R2); 60 MB медиа лежат в git и в каждой сборке — вынести на R2 через `VITE_ICONOGRAPHY_ASSET_BASE` (free tier: 10 GB, egress $0), это гигиена репо и разгрузка Vercel-трафика (100 GB/мес), не срочно; для РФ-зеркала R2 за Cloudflare не годится (Cloudflare блокирован у части провайдеров), картинки остаются на Hosting, проект на Blaze — сверх 360 MB/сутки $0.15/GB; согласовать поддержку, targeted deploy `sendFeedback`; внешняя редактура 83 записей (46 русских, 7 грузинских) и 12 статей о школах, дальнейшее расширение GE и храмовых росписей, замена трёх малых календарных фрагментов более качественными копиями, первичная сверка пяти критских музейных записей, сверка датировки Николая из Эрмитажа `ru-191028144`, доменное SEO и проверка РФ-доступности. См. `docs/guides/iconography.md` и отчёт 2026-09-10. |
...
| LP-18 | L (S) | Хвосты курсов и доступа (2026-09-23) | Причина провала автодобавления в «Все» (27.04–05.07), неиспользуемые `featuredCourseIds` у «Все», нестабильный `UserMenu.research.test.tsx` |
| AC-3 | M (S-M) | Автор курса не узнаёт о заявке на доступ | Заявки (AC‑2) видны автору только на `/admin/students?course=…`: в кабинете автора счётчика нет, Telegram шлёт в единственный чат владельца, писем нет. Шаг 1 — показатель «Заявки» в карточке курса кабинета (~0.5 ч, индекс не нужен). Шаг 2 — канал уведомления автору (почта / персональный Telegram / суточный дайджест), решать после пилота: способа связи автора в профиле сейчас нет. Отражено в карте внешних курсов |
...
| TS-1 | M (M) | Тесты курсов: новые типы вопросов | Сопоставление колонок и упорядочивание. Сначала схема + экран прохождения, авторинг через JSON (`publishTestFromJson.ts`), редактор не трогать; упрощение редактора — отдельным шагом |
| TS-2 | M (M) | Тест после лекции + таблица лидеров | Тост «пройди тест по занятию» на ~90% просмотра (прогресс уже есть в `StudyVideoPlayer`), лидерборд по `testResults` |
| TS-3 | M (S-M) | Аудит старых тестов dev-курса (этап 3d) | Вердикты по курсовым и ноябрьским тестам; первым делом снизить порог 100% на «Ребусах», из-за которого лучший тест «Термины в контексте» никто не прошёл |
| TS-4 | L (M) | Идеи тестов вне лекций | Каталог идей для курсовых и тематических тестов — в разделе «Тесты курсов» ниже |
...
- **Workaround сейчас (использован для wave-8):**
  ...
  ```
- **Задачи:**
  - [ ] Синхронизировать `firestore.indexes.json` с реальной БД. **Сверка 2026-09-22 (`npx firebase firestore:indexes` против файла): расхождение уже не 4 индекса, а по одному в каждую сторону** — в проде есть `books: active+status+title`, которого нет в файле; в файле висит `searchHistory: userId+createdAt DESC`, которого нет в проде (мёртвая запись — `useSearchHistory` сознательно запрашивает без `orderBy`, см. комментарий в хуке, живого бага нет). Остальные 12 индексов и единственный `fieldOverride` совпадают. То есть работы здесь на 20 минут: дописать `books`, удалить `searchHistory`.
  - [ ] Перепроверить симптом 1 на firebase-tools 15.x — баг с `__name__` у vector-индексов ловили на 14.22.0, CLI с тех пор бампнут (2026-08-29). Тест деплоя индексов делать глазами и без `--force`: CLI предлагает удалять индексы, которых нет в файле.
...
- [ ] **Follow-up'ы режима `--prod-data`** (ревью 2026-08-29, не блокеры): (а) PII-маска ложно срабатывает на числах в текстах вопросов («незнакомый телефон» в тесте на рабочую память лишился данных) — рассмотреть allow-паттерны; (б) обычный прогон после `--prod-data` наследует прод-доки песочницы (future count-ассерты станут зависеть от истории) — либо `--reset` по умолчанию при смене режима, либо метка режима в песочнице; (в) снапшот без formatVersion, битый JSON падает голым SyntaxError; (г) protected core-доки глушат единственный документ с `intro` — прод-форму intro кабинет в стенде не видит; вариант: merge-импорт полей, которыми фикстура не владеет; (д) хардкоды имён core-курсов остались в `admin-empty.spec.ts` и `author.spec.ts` — перевести на `SMOKE_COURSES`, как superadmin/student-group.
...
- [x] **Заявки живут в Firestore и обрабатываются в админке** — ✅ 2026-09-21. Коллекция `accessRequests/{id}` (auto-id): create — автору заявки со статусом `new` и строгим `hasOnly` по семи ключам; read/update — супер-админ, со-админ и админ курса из `courseId` (менять можно только `status`/`resolvedAt`/`resolvedBy`); delete — супер-админ. Гость шлёт заявку своим окном (`src/pages/home/AccessRequestModal.tsx`) с выбором курса и пунктом «не знаю / несколько»: сначала `addDoc`, потом Telegram тем же `submitFeedback` — best effort, его ошибка заявку не отменяет. Пока заявка `new`, кнопка на `/home` заменяется статусом «Заявка отправлена dd.mm.yyyy · ждём ответа», дубль не отправить. В админке — общая панель `src/components/accessRequests/AccessRequestsPanel.tsx`: на `/admin/users` все новые заявки, на «Студентах курса» — только заявки своего курса; «Открыть курс» = `bulkEnrollStudents` + закрытие заявки, «Отклонить» подтверждается на месте. Индексы не трогались: только equality-фильтры, сортировка на клиенте (MR-5 ниже). +22 кейса в `tests/integration/firestoreRules.test.ts` (158 зелёных), +21 юнит, ролевой стенд 50/50 с `--with-functions` (новый сценарий `student-no-access`). Доки: [firestore-schema.md](../reference/firestore-schema.md#accessrequestsrequestid-ac-2), [multi-course.md](../guides/multi-course.md), [feedback-system.md](../guides/feedback-system.md), [testing-system.md](../guides/testing-system.md), [routes.md](../reference/routes.md). Rules задеплоены 2026-09-22, клиент — PR #83.
- **Зависимости:** страница `/admin/users` (вкладки, карточка) и `/admin/students` уже есть; rules-тесты по образцу `feature_events`.

### AC‑3. Автор курса не узнаёт о заявке на доступ (P: M, E: S-M)
- **Контекст (2026-09-22, вопрос Алексея после выката AC‑2):** заявки хранятся и обрабатываются, но автор курса видит их, только если сам откроет `/admin/students?course=…`. Уведомления нет нигде: в кабинете автора (`AuthorCabinet`) три показателя — занятия, вопросы студентов, события за 4 недели, заявок среди них нет; Telegram-уведомление `sendFeedback` уходит в один чат владельца (`TELEGRAM_CHAT_ID`, адреса на автора нет); писем по заявкам не шлётся. Пока это чинится вручную: владелец пересылает заявку автору.
- [ ] **Шаг 1 — счётчик в кабинете (P: M, E: S, ~0.5 ч).** Четвёртый `StatBlock` «Заявки» в карточке курса `src/pages/admin/cabinet/AuthorCabinet.tsx` + метрика в `useAuthorCabinetStats` (один `getDocs` по `accessRequests` с `where('courseId','==',id)` + `where('status','==','new')`, equality-фильтры — индекс не нужен), ссылка ведёт на `/admin/students?course=…`. Новых коллекций и полей не заводит; упавшая по правам метрика показывается прочерком, как соседние.
- [ ] **Шаг 2 — канал уведомления (P: M, E: M).** Нужен способ связи автора, которого в профиле нет. Варианты: (а) e-mail на адрес аккаунта через существующий почтовый контур (проверить, есть ли он — сейчас письма шлёт только booking); (б) персональный Telegram: поле `telegramChatId` в профиле + выбор чата в `functions/src/lib/telegram.ts` вместо одного `TELEGRAM_CHAT_ID`; (в) дайджест раз в сутки вместо мгновенного — дешевле и не шумит. Решать после пилота: до него авторов единицы и ручная пересылка дешевле кода.
- **Зависимости:** AC‑2 (коллекция и панель уже есть). Отражено в карте внешних курсов (этапы 3 и 4).

## 💤 Low Priority

### LP‑18. Хвосты курсов и доступа (P: L, E: S)
- **Контекст:** всплыло 2026-09-23 при правках каталога, «актуальных» курсов и карточки пользователя.
- [ ] **Почему 38 пользователей не попали в «Все».** Зарегистрированы 2026-04-27…07-05; `onUserCreate` добавляет в группу с коммита `0fbf904` (2026-04-25), все регистрации с 2026-07-21 в группе есть. Добиты `scripts/ensure-everyone-group.ts --apply` (89 → 127). Гипотеза — функция с этим кодом попала в прод позже коммита; проверить логи `onUserCreate` за тот период на «Failed to add … to 'everyone'», если логи ещё хранятся. Если причина иная — повторный прогон скрипта безопасен (merge).
- [ ] **У `groups/everyone` в проде лежат `featuredCourseIds` (`clinical`, `general`)** — в расчёте актуальных больше не участвуют, поле в редакторе скрыто. Удалить при случае (Admin SDK, `FieldValue.delete()`), чтобы не путало при чтении базы.
- [ ] **`src/components/__tests__/UserMenu.research.test.tsx` нестабилен в полном прогоне** («открывает и закрывает Drawer по кнопке и Esc» падал 1 раз из 1 в `npx vitest run`, отдельно зелёный и на ветке, и на `main`). Похоже на таймаут под нагрузкой — поднять таймаут `waitFor` или убрать зависимость от анимации.
...
Серия тестов «Психологии развития» (июль–сентябрь 2026): цепочки база → кейсы → вглубь по каждому лекционному занятию, формат — `docs/guides/testing-system.md`. Ниже — то, что отложено на после серии.

### TS‑1. Новые типы вопросов (P: M, E: M)
- Сейчас единственный тип — один верный из 2–8 вариантов. Нужны сопоставление двух колонок и упорядочивание (например, стадии по возрасту).
- Путь без переделки редактора: тип в схеме `TestQuestion` + экран прохождения + линтер (`scripts/lintTestJson.ts`), авторинг через JSON и `publishTestFromJson.ts`. Упрощение редактора для админов — отдельной задачей.

### TS‑2. Тест после лекции + таблица лидеров (P: M, E: M)
- На ~90% просмотра лекции — ненавязчивое предложение пройти тест по занятию (прогресс-события уже есть в `StudyVideoPlayer`).
- Простая геймификация: таблица лидеров по `testResults` (лучший результат × число пройденных цепочек), с опцией скрыть себя.

### TS‑3. Аудит старых тестов dev-курса (P: M, E: S-M)
- Разбор 2026-09-22 (ничего не правилось, решения за Алексеем): «Ребусы» — на картинке кризиса написан ответ, у «окна» два верных; «Цитаты» — около половины цитат выдуманы или приписаны не тем авторам; «Термины в контексте» — лучший жанр (пропуск в цитате из первоисточника), но пустые пояснения и синонимы в вариантах; «Интегральная» и «Китайская перспектива» — машинная генерация со словами-заполнителями в неверных вариантах и фактическими ошибками; «Узнай психолога» — оставить, добавить Фрейда.
- Порог 100% на «Ребусах» запирает «Цитаты» и «Термины» — снизить при любом решении.
- Старый «Развитие до родов» удалён 2026-09-22 (вытеснен новой цепочкой; бэкап в `~/Downloads/psych-dev-videos/tests/_backups/`, результаты студентов сохранены).
- Дубль интро-лекции в глобальном поиске (две одинаковые карточки).

### TS‑4. Идеи тестов вне лекций (P: L, E: M)
Курсовые:
- «Три жизни» — финальный тест по сквозным персонажам Мира, Тимур и Соня, от зачатия до зрелости.
- Биографии из книг и фильмов по периодам (например, «Отрочество» Линклейтера, снимавшееся 12 лет).
- Межлекционные сравнения: один феномен в разных периодизациях, ссылки на две лекции сразу.

Медиа:
- Детская речь на слух — определить возраст (гуление, лепет, первые слова); источники вроде CHILDES/TalkBank, лицензии согласовать до использования.
- Фото классических экспериментов — на какой возраст рассчитан (зрительный обрыв, «три горы», задачи на сохранение, «незнакомая ситуация»).
- Фрагменты чужих лекций с нужной минуты (`videoUrl` с `?start=` уже поддерживается): что за понятие объясняет лектор.

Пренатальный период:
- «Что умеет плод»: угадать неделю, когда плод начинает слышать, различать вкус, спать циклами (изображения УЗИ со свободной лицензией).
- «Мифы о беременности»: что из этого миф (эффект Моцарта для плода, «кесарёные — безвольные», «стресс — гарантированный вред», «плод слышит маму» — правда).
- «Естественные эксперименты истории»: голодная зима, «Ледяной шторм», беременные 11 сентября, Чернобыль — что показало событие и почему оно ценно для науки.
- «Рождение в разных культурах»: сорок дней после родов, кувада, пеленание, домашние и роддомные роды.
- Тренажёр консультации: реплика беременной клиентки → лучший ответ психолога, реплики из трёх оптик.

Младенчество:
- «Вехи первого года»: упорядочить социальную улыбку, удержание головы, переворот, сидение, ползание, «пинцетный» захват, первые шаги, первое слово (ждёт упорядочивания из TS‑1) + окна ВОЗ: норма — диапазон, ходьба от 8 до 18 месяцев.
- «Как спросить младенца»: метод ↔ что с его помощью узнали — предпочтительный взгляд (Фанц), привыкание, нарушение ожидания (Байярджон), контингентное сосание (Калнинс и Брунер), зрительный обрыв, неподвижное лицо (ждёт сопоставления из TS‑1).
- «Мифы о младенцах»: «на руках избалуешь», «Моцарт делает умнее», «ходунки учат ходить», «развивающие видео учат словам» (ДеЛоуч, 2010), «многоязычие тормозит речь», «младенцы не чувствуют боли», «час бондинга решает всё» — миф; «новорождённый узнаёт голос матери», «младенец различает звуки любых языков» — правда.
- Видео-кейсы по заданию из лекции (найти ролики с детьми 0–1): «что за феномен на записи» — неподвижное лицо, социальная референция, ошибка «А, но не Б», комплекс оживления; `videoUrl` с `?start=`, лицензии согласовать.
- «Рефлексы новорождённого»: описание или фото (Commons) → рефлекс (Моро, Бабинского, хватательный, поисковый, шаговый, Галанта) и срок угасания.
- «Младенчество в разных культурах»: сон с родителями и отдельно (Морелли, 1992: майя и США), ношение на теле, пеленание, кормление по требованию и по часам; фильм «Малыши» (2010): Намибия, Монголия, Токио, Сан-Франциско.
- Тренажёр для работы с молодыми родителями: реплика («я плохая мать», «он манипулирует плачем», «свекровь говорит, что надо…») → лучший ответ; отдельно — послеродовая депрессия и «бэби-блюз» (сроки, признаки, шкала EPDS).
...
### NT-1. Заметки: follow-up после аудита вкладки 2026-09-09 (P: L, E: S)
- **Сделано во второй итерации (2026-09-09):** дата и сортировка по `updatedAt` (легаси без `updatedAt` → `createdAt`), диалог удаления и инлайн-ошибки вместо `confirm`/`alert`, «На таймлайн» только для development, «По занятиям» по порядку курса, `NotesFilter.tsx` удалён.
- **Осталось:** `SaveNoteAsEventButton` внутри всё ещё валидирует через `alert` (тест `SaveNoteAsEventButton.test.tsx` стабит alert) — при касании перевести на инлайн-ошибку и обновить тест.
- **Флейк профиля на стенде — закрыт 2026-09-09:** причиной была не сеть, а правило `aiUsageDaily` без guard'а на `resource == null`: `useByokUsage` подписывается на документ текущего дня, которого до первого AI-запроса нет, правило падало с «Null value error», и Firestore-эмулятор рвал Listen-канал, после чего SDK 12.4.0 падал ассерцией `ca9` → `b815` и все подписки страницы замирали («Загрузка курсов…» в профиле, спек `student-group.spec.ts:29`). Long polling (`experimentalForceLongPolling`) проверен и НЕ помогал — откачен. Фикс: владение по префиксу id + `resource == null ||` в `firestore.rules`, тесты в `tests/integration/firestoreRules.test.ts`; на проде правило раньше просто отдавало permission-denied и засоряло консоль ошибкой `useByokUsage onSnapshot error` у всех, кто в этот день не пользовался AI. Правила задеплоены в `psych-dev-site-prod` 2026-09-09 (`npx firebase deploy --only firestore:rules`, одобрено Алексеем; live-версия сверена через `firebase_get_security_rules`).
