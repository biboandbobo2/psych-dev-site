...
| LS-4 | M (M-L) | Первый визит без auth+Firestore в критическом пути (static JSON / REST без auth) | LCP ≤1,5 с desktop / ≤3 с 4G. Единственная открытая LS-задача (остальной сектор закрыт 2026-08-23); требует архитектурного решения пользователя — варианты в секции LS ниже |
...
> Сектор в основном закрыт: LS-1/LS-2 — 2026-08-18, LS-5/LS-7/LS-3/LS-6 — 2026-08-23 (см. `docs/archive/REFRACTORING_ARCHIVE.md`, «Производительность и данные»). Журнал перезамеров и методика — `docs/reference/perf-metrics.md`. Осталась одна задача:

- **LS-4 (P: M, E: M-L). Первый визит без auth+Firestore в критическом пути.** После LS-5/7/3/6 остаток холодного LCP /home — цепочка `JS → identitytoolkit accounts:lookup (~1,9 с +0,55 с) → Firestore (~2,5 с) → LCP ~3,2 с»: Firestore SDK не шлёт запросы, пока auth не выдаст первый токен, поэтому даже публичные данные ждут auth-инициализацию. **Метрика:** первый визит LCP ≤1,5 с десктоп / ≤3 с throttled 4G; в критическом пути нет `identitytoolkit`.
  **Требует архитектурного решения пользователя** (сверка 2026-08-23; все пути упираются в согласование):
  1. *Static JSON за Cloudflare* (исходная формулировка): endpoint в `api/*` с `s-maxage` + CF edge cache. Новый файл в `api/` требует пересчёта Vercel-лимита (LP-13 фиксировал 11/12; фактических файлов в `api/` сейчас 8 — пересчитать перед решением) или встраивания экшеном в существующий endpoint; генерация при deploy невозможна без секрета в Vercel env.
  2. *Более простой вариант без новой инфраструктуры:* публичные данные первого рендера (`courses`, `courseNavIndex`, `pages/home`) качать напрямую Firestore REST (`?key=<web api key>` без auth-токена, rules публичные) параллельно с auth — убирает `identitytoolkit` из критического пути ценой клиентского парсера REST-формата; хост `firestore.googleapis.com` остаётся (preconnect уже стоит, LS-7).
  3. *CF Worker / Cloud Function onWrite → Storage* — тяжелее по инфраструктуре, отдельное согласование.
  Прод-данные для решения собирать ПОСЛЕ деплоя правил `courseNavIndex` (см. LS-3 в архиве) — он уже уменьшает Firestore-часть цепочки на /home.
