...
|----|----------|-------|-----------------------|
| ICO-1 | M (M) | Иконография: production и рост | Опубликовано 2026-09-11 в составе Академии (`/iconography`, без ссылок с сайта); до анонса вынести 60 MB медиа из `dist` на CDN (R2) через `VITE_ICONOGRAPHY_ASSET_BASE` — иначе РФ-зеркало на Spark (360 MB/сутки) ляжет от полусотни сессий; согласовать поддержку, targeted deploy `sendFeedback`; внешняя редактура 83 записей (46 русских, 7 грузинских) и 12 статей о школах, дальнейшее расширение GE и храмовых росписей, замена трёх малых календарных фрагментов более качественными копиями, первичная сверка пяти критских музейных записей, сверка датировки Николая из Эрмитажа `ru-191028144`, доменное SEO и проверка РФ-доступности. См. `docs/guides/iconography.md` и отчёт 2026-09-10. |
