...
| AD-2 | M (M) | Admin-редактор страницы `/about` | Контент `/about` (миссия + список партнёров) переезжает в Firestore, в админке — редактор по образцу страниц «о курсе» |
| HM-1 | M (S) | Continue-cards: настройка «актуальных» курсов | Поля `featuredCourseIds` у `groups/{id}` и `users/{id}` (max 3), приоритет профиль > группа > последний просмотренный > CTA в профиль |
| HM-2 | M (M) | `/about` → вкладочная структура + страницы проектов | Вкладки «Проект Академия / Команда сайта / Команда академии / История / Партнёры»; шаблон страницы под проект (по образцу `WarmSprings2Page`) |
| HM-3 | M (L) | Супер-админский редактор статических страниц | Расширение AD-2: список редактируемых страниц (`/about`, страницы проектов, партнёры) хранится в Firestore, UI редактора в админке |
