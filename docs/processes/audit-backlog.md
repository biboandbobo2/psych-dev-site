...
### RS‑1. ✅ Wikidata-перевод в обычном поиске (P: M, E: M) - РЕАЛИЗОВАНО (2026-07-17)
- **Реализация отличается от исходного плана:** вместо отдельной кнопки «Глубокий поиск» Wikidata встроена в обычный поиск как fallback перевода (`buildEnglishQuery` в `api/papers.ts`):
  1. Русский запрос переводится словарём `RU_TO_EN_TERMS`.
  2. Если перевод неполный (осталась кириллица) — Wikidata ищет концепт по русскому названию и берёт английский лейбл.
  3. Гибридные RU/EN строки в источники не отправляются (давали мусор/0 результатов); чистый EN-вариант и RU-оригинал ищутся параллельно.
