...
> **Версия:** 1.0
> **Дата:** 2025-11-07
> **Статус:** Текущая архитектура (до рефакторинга)
...
- 📅 **[Система таймлайна](./TimelineGuide.md)** - интеграция через заметки и возрастные периоды
...
├── components/
│   ├── TestEditorForm.tsx       # 1575 строк ⚠️ МОНОЛИТ
│   ├── TestEditorModal.tsx      # 1009 строк ⚠️ МОНОЛИТ
│   ├── QuestionEditor.tsx       # 1126 строк ⚠️ МОНОЛИТ
│   ├── QuestionPreview.tsx      # 312 строк
│   └── TestHistory.tsx          # 196 строк ✅
├── pages/
│   ├── Tests.tsx                # 485 строк - тесты о курсе
│   ├── AgeTests.tsx             # 311 строк - тесты по периодам
│   ├── DynamicTest.tsx          # 778 строк - прохождение теста
│   ├── AuthorsTest.tsx          # 348 строк ⚠️ УСТАРЕЛО
│   ├── AuthorsTestLevel2.tsx    # 415 строк ⚠️ УСТАРЕЛО
│   └── AuthorsTestLevel3.tsx    # 462 строк ⚠️ УСТАРЕЛО
└── constants/
...
- **Всего:** ~8800 строк
- **Из них мёртвый код:** ~1225 строк (AuthorsTest*)
- **Дублирование:** ~150-200 строк
- **Потенциал оптимизации:** 30-40% (до ~5000-6000 строк)
...
### 🔴 TestEditorForm.tsx (1575 строк) - ТРЕБУЕТ РЕФАКТОРИНГА

**Расположение:** `src/components/TestEditorForm.tsx`

**Назначение:** Главная форма редактирования теста.

**Что делает:**
1. Редактирование метаданных теста (название, описание, рубрика)
2. Управление вопросами (добавление, удаление, порядок)
3. Настройка внешнего вида (темы, цвета, градиенты, иконки)
4. Настройка политик показа ответов
5. Выбор prerequisite теста (для цепочек)
6. Импорт тестов из JSON
7. Экспорт тестов в JSON
8. Валидация перед сохранением
9. Сохранение в Firestore

**Props:**
```typescript
interface TestEditorFormProps {
  testId?: string;              // ID теста для редактирования (undefined = создание)
...
1. **Метаданные (строки 200-350)**
   - Поля: title, description, rubric
   - Выбор prerequisite теста
   - Установка requiredPercentage
   - Настройка defaultRevealPolicy

2. **Внешний вид (строки 350-800)**
   - Выбор темы из presets
   - Настройка mainColor
   - Настройка градиентов (background, accent, badge)
   - Выбор иконок (introIcon, badgeIcon)
   - Установка bulletPoints
   - Предпросмотр темы

3. **Вопросы (строки 800-1200)**
   - Список вопросов с drag & drop
   - Добавление нового вопроса
   - Редактирование через QuestionEditor
   - Удаление вопроса
   - Предпросмотр вопроса

4. **Импорт/Экспорт (строки 1200-1400)**
   - Импорт из JSON файла
   - Экспорт теста в JSON
   - Генерация шаблона вопросов

5. **Валидация и сохранение (строки 1400-1575)**
...
**Проблемы:**
- ⚠️ Слишком много ответственности в одном компоненте
- ⚠️ Сложно тестировать
- ⚠️ Длительная компиляция
- ⚠️ Дублирование логики работы с темами

---

### 🔴 QuestionEditor.tsx (1126 строк) - ТРЕБУЕТ РЕФАКТОРИНГА
...
**Проблемы:**
- ⚠️ Слишком много логики в одном компоненте
- ⚠️ Смешана логика загрузки файлов и UI
- ⚠️ Нет переиспользования кода (например, для списков ресурсов)
...
### TestEditorModal.tsx (1009 строк) - ТРЕБУЕТ РЕФАКТОРИНГА

**Расположение:** `src/components/TestEditorModal.tsx`

**Назначение:** Модальное окно-обёртка над TestEditorForm.

**Что делает:**
- Открывает/закрывает модалку
- Рендерит TestEditorForm внутри
- Обрабатывает callbacks (onSave, onCancel)
- Управляет состоянием открытия
...
**Проблемы:**
- ⚠️ Слишком большой для простой обёртки
- ⚠️ Много дублирующейся логики с TestEditorForm
- ⚠️ Возможно, стоит объединить с TestEditorForm после рефакторинга

---

## Страницы приложения

### Tests.tsx (485 строк) - Тесты по всему курсу

**Расположение:** `src/pages/Tests.tsx`  
**URL:** `/tests`

**Назначение:** Страница со списком тестов по всему курсу психологии развития.

**Что отображает:**
1. Активные тесты из Firestore (с рубрикой `'full-course'`)
2. Заглушки будущих тестов (5 карточек с меткой "Скоро")

**Ключевые функции:**

```typescript
// Фильтрация тестов по рубрике
...
**Система цепочек:**
- Использует `buildTestChains()` для создания иерархии тестов
- Корневой тест = тест без prerequisite
- Уровни = тесты, ссылающиеся на корневой через prerequisiteTestId

**Пример цепочки:**
```
Тест "Авторы психологии развития"
  ├─ Уровень 2: "Продвинутый уровень"
  └─ Уровень 3: "Экспертный уровень"
```

**Отображение карточек:**
- Заголовок извлекается из `title` (часть до `:`)
- Цвета и иконки из `appearance`
- Показывается количество вопросов
- Показывается количество уровней
- Для уровней - кнопки с названиями или "🔒" если заблокировано

**Заглушки (COURSE_TESTS):**
```typescript
const COURSE_TESTS: LegacyTest[] = [
  { title: 'Общие знания по курсу', icon: '📚', ... },
  { title: 'Этапы развития', icon: '🌱', ... },
  { title: 'Практическое применение', icon: '🎯', ... },
  { title: 'Когнитивное развитие', icon: '🧠', ... },
  { title: 'Социально-эмоциональное развитие', icon: '❤️', ... },
];
```
Отображаются с меткой "Скоро" и opacity-60.

**Порядок отображения:**
1. Активные тесты (новые → старые)
2. Заглушки

---

### AgeTests.tsx (311 строк) - Тесты по возрастным периодам

**Расположение:** `src/pages/AgeTests.tsx`  
**URL:** `/tests/age-periods`

**Назначение:** Страница со списком тестов по конкретным возрастным периодам.

**Что отображает:**
- Тесты с рубриками из AgeRange (prenatal, infancy, toddler и т.д.)
- Без заглушек
...
**Особенности:**
- Используется та же логика цепочек (`buildTestChains`)
- Карточки отображаются аналогично Tests.tsx
- Уровни показывают реальные названия (не "Уровень 2", а название из title)

**Функции форматирования:**
```typescript
// Очистка названия от префикса "Уровень N"
function cleanLevelLabel(text: string) {
  return text.replace(/^Уровень\s*\d+\s*[-–—:]?\s*/i, '').trim();
}

// Извлечение названия уровня
function formatLevelLabel(test: FirestoreTest, index: number) {
  ...
  return cleanLevelLabel(test.title) || `Уровень ${index}`;
}
```

**Проблемы:**
- ⚠️ Дублирование кода с Tests.tsx (~150-200 строк)
- ⚠️ Одинаковые функции: cleanLevelLabel, formatLevelLabel, buildTestChains, getTestMetadata
