...
├── components/
│   ├── TestEditorForm.tsx       # 1033 строки 🟡 УЛУЧШЕНО (было 1575)
...
├── pages/
│   ├── Tests.tsx                # ~420 строк 🟡 УЛУЧШЕНО (использует TestCard)
│   ├── AgeTests.tsx             # ~280 строк 🟡 УЛУЧШЕНО (использует TestCard)
│   ├── DynamicTest.tsx          # 778 строк ⚠️ ТРЕБУЕТ РЕФАКТОРИНГА
