...
Гейт (`scripts/check-file-sizes.cjs`) стоит в pre-commit и входит в `npm run validate`:

- **новый** файл > 500 строк блокирует коммит;
- легаси-список заморожен внутри скрипта (2026-07-17, 9 файлов) — эти файлы дают
  только warning и рефакторятся по мере касания, добавлять в список ничего нельзя;
- когда легаси-файл похудел ниже лимита, скрипт подсказывает убрать его из списка.

**Исключения (разрешено > 500):**
- Типы и интерфейсы (`types.ts`)
- Константы и данные (`constants.ts`, `data/`)
- Конфигурация (`config.ts`, `vite.config.js`), тесты

---

## Структура проекта

### Фактическая структура — гибрид `features/` + `pages/`

Проект осознанно живёт в гибриде; полная миграция «всё в features/» не планируется.

```
src/
├── features/        # Самодостаточные фичи (researchSearch, lectureSearch,
│   └── <feature>/   #   periods, contentSearch, bookSearch, ...)
│       ├── components/
│       ├── hooks/
│       ├── lib/ | utils/
│       ├── types.ts
│       └── index.ts # Barrel export
├── pages/           # Страницы-роуты (+ подпапки крупных страниц:
│                    #   pages/timeline/, pages/disorderTable/, ...)
├── components/      # Общие переиспользуемые компоненты
├── hooks/           # Общие хуки
├── stores/          # Zustand stores
├── lib/             # Инфраструктура (Firebase, debug, API-клиенты)
├── utils/           # Общие вспомогательные функции
├── types/           # Глобальные типы
└── data/            # Статичные данные (тяжёлые — только dynamic import)
```

### Правила организации

1. **Новая многофайловая фича** → `src/features/<feature>/` с barrel export.
2. **Логика конкретной страницы** → подпапка рядом со страницей (`pages/<page>/`).
3. **Утилиты/типы фичи** → внутри фичи; **общие** (2+ фичи) → `src/utils/`, `src/types/`.
4. Не выносите в общие папки то, чем пользуется одна фича, — и наоборот.
...
### Названия и иконки курсов

Единственный источник имён и иконок курсов в UI — `useCourses()` / `courseMap`
(живые данные из `courses/{courseId}`). Статические помощники
(`CORE_COURSE_META`, `getCourseDisplayName`, `getCourseIcon`) — только fallback
на время загрузки: они не знают динамических курсов и устаревают при
переименовании core-курсов. Не денормализуй имя курса в другие коллекции и не
заводи локальные словари названий в компонентах (прецеденты: транслит-ID в
`LectureSelector`, устаревший бейдж в `ContentSearchResults` — исправлены
2026-08-28).
...
Аудитория — студенты, значительная часть учится с телефона. Минимальные правила:

- Новые UI-фичи проектируются responsive по умолчанию; desktop-only допустим
  только осознанно (как selection-меню в конспекте) и с пометкой в коде/доке.
- Перед мержем UI-фичи — смоук на узком вьюпорте (≤ 430px), хотя бы в devtools.
- Hover-only взаимодействия обязаны иметь тач-эквивалент или деградацию.
...
**Версия:** 2.1 — фактическая гибридная структура, живой гейт check:file-sizes, мобильный минимум
