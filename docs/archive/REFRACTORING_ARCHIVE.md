...
- **5. Тестирование и QA.** Добавлен `npm run test:ci`, настроен CI (lint → test → build → e2e), заведена интеграционная инфраструктура на Firebase эмуляторах (`tests/integration/**`, `test:integration`). Playwright настроен, `tests/e2e/production-smoke.spec.ts` покрывает базовые сценарии (запуск локальный; утверждение «запускается в CI» было неверным — в `ci.yml` playwright-шага никогда не было, зафиксировано 2026-08-18), traces сохраняются. Скрипт `scripts/check-module-initialization.cjs` устранил 8 module-init warning'ов; `docs/architecture/guidelines.md` описывает политику тестирования загрузки модулей.
...
- **LS‑4. Первый визит без ожидания Firebase Auth** (закрыта 2026-08-23, PR #73; сектор LS этим закрыт целиком). Исходная формулировка «static JSON за Cloudflare» заменена согласованным с пользователем более простым вариантом: Firestore SDK не шлёт запросы до инициализации auth, а гостевой /home целиком ждёт `useCourses` — поэтому публичные данные первого рендера (`courses`, `courseNavIndex`) теперь качаются прямым REST-fetch (`src/lib/firestorePublicRest.ts`: декодер typed values + таймаут; правила публичные, ключ не нужен) параллельно с auth. SDK остаётся источником истины и перезаписывает REST-результат; ошибка REST-пути молча оставляет прежнее поведение; в vitest REST выключен глобально (`MODE === 'test'`). Эффект (замер настоящего гостя — чистый браузер без сессии, тем же скриптом до/после): /home LCP **1,92 → 1,28 с** (цель ≤1,5), первый запрос данных 1,2 с → 0,15 с от старта JS. Незанятые альтернативы (endpoint за CF-кэшем, CF Worker) остаются возможной надстройкой, если понадобится выжать ещё для РФ.
- **LS‑5. Лишние modulepreload / статические рёбра entry→page-чанки** (закрыта 2026-08-23, PR #70). Диагноз оказался глубже формулировки бэклога: удалить preload-теги было недостаточно — страничные правила `chunkMapper` в `vite.config.js` засасывали общие модули (110 штук в admin-чанке: firebase, stores, usePeriods, UI-кит) в page-чанки, и entry статически импортировал admin/tests/timeline/profile/research/booking. Фикс: страничные правила удалены (страницы режет сам Rollup по dynamic imports из `lazy.ts`; catch-all-вариант «app-чанк» замерен и отвергнут — 560 КБ против 336), остались vendor / shared-constants (история TDZ) / event-icons; дроверы поиска и AI в `UserMenu` переведены на lazy. Эффект: статический граф entry 515 → 321 КБ gzip, modulepreload 11 → 2, прод-замер JS на старте 353 КБ (холодный, с чанком /home). Цель бэклога «≤250 КБ» недостижима без сплита vendor (252 КБ: react+firebase+router — всё стартовое). Попутно: e2e-смоук timeline-чанков переведён на публичный `/_timeline/automation` — на `/timeline` за RequireAuth он был зелёным только из-за самого бага.
- **LS‑7. Self-host шрифтов + preconnect** (закрыта 2026-08-23, PR #70). Бэклог отставал: preconnect к fonts.googleapis уже стоял, Manrope/Sofia Sans уже self-hosted через @fontsource-variable, но index.html держал дубль Manrope с cdn.jsdelivr.net и Playfair Display с fonts.googleapis, а лендинг — второй дубль Manrope в Helmet. Всё убрано; Playfair теперь `@fontsource-variable/playfair-display`, импортируется только лендингами (vozrast, warm_springs2), в их CSS семейства переведены на `Playfair Display Variable` / `Manrope Variable`. Вместо шрифтовых preconnect — preconnect к identitytoolkit и firestore.googleapis.com (реальный критический путь auth→Firestore). Внешних хостов в критическом пути: 0 (проверено на обоих лендингах).
- **LS‑3. Nav-индекс курса + фикс вечного зависания шторки занятий** (закрыта 2026-08-23, PR #71). Коллекция `courseNavIndex/{courseId}` — маленький док (id/title/order опубликованных занятий + флаг `courseOpen`; факт 0,1–1,8 КБ на курс при цели ≤10 КБ). `useCourseNavItems`: сначала индекс (1 doc-read), fallback — полная коллекция; SWR-кэш в localStorage (`nav-index:{courseId}`); всё под таймаутом 8 с — раньше зависший `getDocs` застревал в in-flight кэше `LESSON_REQUESTS` навсегда («вечная Загрузка занятий…», наблюдалось вживую). `useCoursesOpenness` (гостевой /home качал полные коллекции занятий ВСЕХ курсов ради индикатора 🔓) переведён на готовый `courseOpen` из индекса. Пересборка — админ-хуки `useContentSaver`/`useReorderLessons` через `rebuildCourseNavIndex`; бэкфилл `scripts/backfillCourseNavIndex.ts` выполнен (7 курсов; ловушка: в `courses` есть доки-дубли core-курсов без lessons — исключены, иначе затирали бы core-индексы пустыми). Rules: read public, write `canEditCourse(courseId)` + 4 регрессионных кейса в `firestoreRules.test.ts`. Схема — `docs/reference/firestore-schema.md`.
- **LS‑6. Статический сплеш в index.html** (закрыта 2026-08-23, PR #72). Инлайн-разметка в `#root` (только inline-стили — CSS-бандл ещё не загружен): плитка Ψ + «DOM Academy» + спиннер в цветах theme.css; виден с первого paint (~TTFB), React заменяет через `createRoot().render()`. Заодно удалён осиротевший `LoadingSplash` — после LS-2 не использовался нигде и нёс устаревший бренд «Psych-Dev». Скелетон-layout и ротация фактов из идеи бэклога сознательно не делались (после LS-2/LS-5 React стартует быстро, спекулятивная сложность).
...
- **HP‑1. Nightly интеграционные тесты + воскрешение CI** (закрыта 2026-08-18). Главное, чего бэклог не знал: CI был красным ~100 прогонов подряд с 2026-04-28, единственной защитой был pre-push `validate`. Причины: (1) корневой vitest собирал `tests/integration` в дефолтный прогон — без эмуляторов падение с ECONNREFUSED; (2) Functions Checks подхватывал корневой `vitest.config.ts` (`ERR_MODULE_NOT_FOUND` — фикс MR‑9 был неполным). Решение: условный include по `VITEST_INTEGRATION=1` в `vitest.config.ts` (юниты и integration разведены), собственный `functions/vitest.config.ts` (node env), root job переведён на `npm run test:ci`. Nightly: `.github/workflows/nightly-integration.yml` — cron 02:30 UTC + workflow_dispatch, temurin 21, `npm run test:integration`, артефакт emulator-логов при падении, Telegram-нотификация (GH-секреты скопированы из GCP Secret Manager). Service-account secret не понадобился — тесты идут на эмуляторах. Раздел «как читать nightly-прогоны» — в testing-system.md.
...
- **MR‑9. Functions Checks CI** (сверка 2026-07-11). `working-directory: functions` + `npm ci --include=dev` + локальный `vitest` в `functions/package.json`. **Поправка 2026-08-18:** фикс был неполным — без собственного `functions/vitest.config.ts` vitest поднимался вверх и грузил корневой конфиг (`ERR_MODULE_NOT_FOUND: vitest`), job оставался красным до HP-1. Закрыто добавлением `functions/vitest.config.ts`.
