...
Rules: read — только владелец, причём и для **несуществующего** документа (клиент
подписывается на текущий день до первого AI-запроса): владение проверяется по
префиксу id (`docId.split('_')[0] == request.auth.uid`), `resource.data.uid` — только
если документ есть. Без guard'а на `resource == null` правило падало с «Null value
error», а в эмуляторе это рвало Listen-канал целиком (audit-backlog NT-1). Write — только Admin SDK.
...
Личные заметки пользователей: ручные, лекционные конспекты (режим конспекта) и timeline-заметки. Источник истины по типам — `src/types/notes.ts`, запись — `src/hooks/useNotes.ts`.
...
interface Note {
  ...
  title: string;
  content: string;                // Плоский текст (для lecture — без таймкодов)

  ageRange: AgeRange | null;      // Legacy-период курса development (или null)
  periodId: string | null;        // ID занятия
  periodKey: string | null;       // "courseId::periodId" — ключ группировки
  periodTitle: string | null;
  courseId: string | null;

  noteScope: 'manual' | 'lecture' | 'timeline';

  // Только для noteScope='lecture' (режим конспекта):
  lectureVideoId: string | null;  // YouTube id или URL видео
  lectureKey: string | null;      // "courseId::periodId::videoId"
...
### `lectureQuestions/{questionId}`

Вопросы студентов к семинару по занятиям курса. Задаются из режима конспекта (с таймкодом момента видео) или со страницы занятия. Клиентские запросы — только equality-фильтры (composite-индексы не нужны).
...
interface LectureQuestion {
  ...
  videoId: string | null;         // YouTube id, если задан из оверлея
  startMs: number | null;         // Момент лекции
...
**Правила доступа:**
- read: автор; лекторы курса (`canEditCourse(courseId)` по custom claim `editableCourses`); участники группы `groupId` при `visibility='group'`
...
interface SharedLectureNote {
  ...
  lectureTitle: string | null;
  videoId: string | null;
  segments: LectureNoteSegment[]; // 1..100 (правила)
  visibility: 'group' | 'lecturers';
  groupId: string | null;
  createdAt: Timestamp;
}
...
interface VideoTranscriptDoc {
  ...
  source: 'youtube' | 'manual';   // manual = стенограмма студийного качества (importManualTranscript.ts); refresh её не трогает
