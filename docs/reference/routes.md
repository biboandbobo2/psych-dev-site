...
| `/home` | `HomePage` | Домашний дашборд: continue-курсы, лента объявлений/событий, каталог курсов, выезжающий drawer со списком занятий, блок «Возможности платформы». Зарегистрированный гость без доступов видит `RegisteredGuestHome` (ветвление — `useGuestStatus`): там кнопка «Запросить доступ» открывает окно заявки с выбором курса и пишет её в `accessRequests` (AC-2). Пока заявка в статусе `new`, вместо кнопки — «Заявка отправлена dd.mm.yyyy · ждём ответа», дубль отправить нельзя; после отказа кнопка возвращается. |
