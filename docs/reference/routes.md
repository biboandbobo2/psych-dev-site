...
|---------|-----------|----------|
| `/` | `Profile` | Профиль (или редирект на `/homepage` для неавторизованных) |
| `/homepage` | `HomePage` | Главная страница с описанием курсов и CTA-кнопками |
...
**Layout:** `BookingSectionLayout` — standalone layout (Sofia Sans, DOM-цвета), отдельный от основного приложения.
