...
| 2026-08-23 | LS-5+7+3+6 (сектор, кроме LS-4) | Полухолодный /home (кэш только vendor): LCP 3,08 с; **FCP 1,13 с** — сплеш LS-6 ждёт render-blocking CSS, на холодном канале это ~1,1 с, не TTFB | /home **FCP 0,34 с** (сплеш ✓ цель LS-6), LCP 1,2–1,8 с (разброс Firestore) | без изменений после LS-5 (~350 КБ полухолодный) | Итог захода 2–3. Правила courseNavIndex задеплоены 2026-08-23 после одобрения пользователя: индекс публично читается (REST-проверка без auth), шторка занятий на проде работает через индекс. Перезамер после деплоя правил: /home повторный LCP 1,18 с / FCP 0,22 с — на LCP авторизованного /home эффекта нет (его элемент — карточки курсов, ждущие auth-цепочку; выигрыш индекса — reads гостевого /home и шторка). Остаток холодного LCP — auth-цепочка (identitytoolkit на 1,4–1,9 с +0,5 с) → LS-4 |
| 2026-08-23 | LS-4 — **сектор LS закрыт целиком** | Настоящий гость (чистый браузер, без сессии и кэша): /home LCP **1,92 → 1,28–1,29 с** (первый прогон после деплоя 1,81 с — холодный CF edge), FCP 0,52–0,55 с. Цель ≤1,5 с десктоп — достигнута | тёплые сценарии без изменений (см. строку выше) | без изменений (~350 КБ) | Вариант 2 развилки: публичные `courses`/`courseNavIndex` качаются прямым Firestore REST-fetch (без auth-токена) параллельно с инициализацией auth — SDK остаётся источником истины и перезаписывает. Замер гостя — скрипт measure-guest (chromium, чистый контекст); baseline гостя 1,92 с снят тем же скриптом до деплоя. 4G не мерили. Дальше по бэклогу — PT-1 |
