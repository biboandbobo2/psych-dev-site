...
- **Центральный холст**:
  ...
  - События — окружности с подписью, цвет по сфере; «решение» — внешнее кольцо цвета сферы; у события с заметками — точка-индикатор; hover — мягкая тень (`.tl-node` в `index.css`).
  - Ветка (edge) отходит от события единым path с плавной дугой в стыке; у выбранного события появляется кнопка `+` (создать ветку, `data-layer="ui"` — не попадает в экспорт).
  - Полосы периодизации — бледная подложка с курсивными названиями периодов вдоль левого края.
  - Внизу — легенда сфер (`TimelineSphereLegend`): клик подсвечивает одну сферу, остальные приглушаются (`sphereFilter`).
...
- **Правая панель** (`fixed right`):
  ...
  - Панель контекстная (2026-07): без выбора — кнопка «+ Новое событие» и карточка-подсказка; выбрано рождение — только форма рождения; событие — его форма; ветка — редактор ветки.
