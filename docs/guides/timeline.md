...
- **Центральный холст**:
  - Вертикальная линия жизни по центру (`LINE_X_POSITION = 2000`): прошедшее — толстая насыщенная, будущее — тонкий бледный пунктир (UX-обновление 2026-07).
  - Шкала возраста каждые 5 лет: десятилетия заметные, пятилетки едва видимые; тёплый фон `#fffdf8`. Слой сетки помечен `data-layer="grid"` (постер-экспорт его убирает).
  - События — окружности с подписью, цвет по сфере; «решение» — внешнее кольцо цвета сферы (только у событий без пиктограммы); у события с заметками — точка-индикатор; ховер-`<title>` расшифровывает маркеры; hover — мягкая тень (`.tl-node` в `index.css`).
  - Ветка (edge) отходит от события единым path с плавной дугой; новая ветка создаётся со смещением наружу от главной линии (дуга видна с первой ветки). Название ветки (`edge.label`, дефолт — имя origin-события) подписано над вершиной. У выбранного события — кнопка `+` (создать ветку); сверху ветки — «хвостик» для изменения длины перетаскиванием (оба `data-layer="ui"` — не попадают в экспорт).
  - **Двойной клик** по главной линии или ветке — `quickCreateEvent`: событие в точке клика (возраст из координаты, кламп в окно), сразу открыто для переименования.
  - **Drag события**: ось фиксируется по первому движению — горизонталь двигает поддерево (дерево топологии), вертикаль меняет возраст (шаг 0.1, кламп в окно ветки/[0, ageMax]); origin ветки вертикально не двигается.
  - Полосы периодизации — бледная подложка с курсивными названиями периодов вдоль левого края.
  - Легенда сфер (`TimelineSphereLegend`) — в левой панели: клик подсвечивает одну сферу, остальные приглушаются (`sphereFilter`).
  - Стартовый оверлей пустого холста (`TimelineStartOverlay`): «Начать с рождения» / «Биография» / «Пример» (`EXAMPLE_TIMELINE`) / «Чистый лист».
  - Уведомления — неблокирующий `TimelineToast` (валидаторы хуков получают `notify`, дефолт alert для тестов); после удаления события — плашка «Удалено N · Отменить» (undo).
- **Правая панель** (`fixed right`):
  - Заголовок с индикатором статуса сохранения (idle/saving/saved/error) и селектором периодизации; при ошибке — плашка «Не удалось сохранить · Повторить» (`retrySave`).
  - Панель контекстная и показывает **актуальный** выбор (2026-07): без выбора — «+ Новое событие», «⚡ Несколько событий сразу» и карточка-подсказка; рождение — только форма рождения; событие — его форма; ветка — редактор ветки (контекст origin, название, длина) + bulk-кнопка. Выбор ветки при открытом редакторе события применяет недописанные правки (flush) и переключает панель.
  - **Авто-применение** (2026-07): правки существующего события и длина/название ветки применяются сами с дебаунсом ~0.8с (`handleFormSubmit(..., { keepFormOpen })` + `markSaved`); кнопка осталась только у создания нового события. Страховка — undo.
  - Блок Undo/Redo — вверху панели.
  - Экспорт: JSON / PNG / **Постер (PNG)** (`exportTimelinePoster` — без сетки, заголовок, легенда сфер, футер) / PDF.
...
- **Событие (`NodeT`)**
  ...
  - `branchId` (2026-07, RFC `docs/plans/timeline-branch-id-rfc.md`) —
    ссылка на ветку-родителя (`EdgeT.id`); `undefined` = главная линия.
    **Источник истины о принадлежности** события ветке: `buildTimelineTree`
    группирует по ссылке, координата — fallback для legacy-узлов без неё
    (normalize досеивает ссылку при загрузке).
  - `x` — горизонтальная координата (чистая презентация).
  - `parentX` — x родительской линии (презентация; для узлов с `branchId`
    normalize держит его равным `edge.x`).
- **Ветка (`EdgeT`)**
  - `id`, `x`, `startAge`, `endAge`, `color`, `nodeId` (событие-источник).
  - `label` (опционально, 2026-07) — название ветки; если не задано, UI показывает название origin-события (работает и для веток биографического импорта).
- **TimelineData**
