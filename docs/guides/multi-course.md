...
**Важно:** создать новый курс может только super-admin — у обычного админа ID
ещё не существующего курса нет в claim `editableCourses`, и rules отклонят запись.

### Скрытие и удаление динамического курса

> **Дата добавления:** 2026-08-18

Оба действия — в панели редактирования курса (карандаш ✏️ в `AdminCourseSidebar`),
доступны только для динамических курсов (core-курсы скрыть/удалить нельзя):

- **Скрытие:** чекбокс «Курс виден студентам» → `courses/{courseId}.published`.
  Скрытый курс пропадает из студенческих списков (`useCourses` фильтрует
  `published === false`), в админ-сайдбаре остаётся с бейджем «Скрыт».
  Прямые ссылки на занятия (`/course/{id}/...`) при этом продолжают работать —
  у занятий свой флаг `published`.
- **Удаление:** кнопка «Удалить курс» с confirm. Клиент удаляет все документы
  `lessons` и сам документ курса одним `writeBatch` (лимит батча 500 — для
  реальных курсов достаточно). Хвосты (`courseAccess` у студентов,
  `editableCourses` в claims админов) не чистятся — они безвредны, но при
  желании убираются вручную.

### Консистентность списка курсов между компонентами

У каждого потребителя `useCourses` — свой экземпляр состояния. Чтобы после
создания/переименования/скрытия/удаления курса списки обновлялись во всех
компонентах сразу (сайдбар, `AdminContent`, модалки), `reload()` из хука
вызывает модульный `invalidateCourses()`, который перечитывает данные во всех
живых экземплярах. До 2026-08-18 обновлялся только экземпляр сайдбара, из-за
чего `useActiveCourse` в `AdminContent` считал свежесозданный курс «неизвестным»
и сбрасывал выбор на первый курс до перезагрузки страницы.
