...
**Публичные страницы раздела:** `/booking` (бронь), `/booking/photos` (фотографии кабинетов), `/booking/pricing` (стоимость), `/booking/directions` (адрес и маршрут к DOM).
...
- Навигация: стрелки ← → (текущая неделя + 8 вперёд)
...
BookingConfirmation автоподставляет имя/фамилию (из `user.displayName` через `parseDisplayName`),
email (из Firebase user), phone (из Firestore).

### Требование фамилии (hard gate + soft prompt)
Бронирование требует, чтобы у пользователя были и имя, и фамилия (≥ 2 символа каждое).
- **Hard gate**: `BookingConfirmation` валидирует оба поля и блокирует submit без фамилии.
  При успешном вводе пишет `displayName = "Имя Фамилия"` в Firebase Auth (`updateProfile`)
  и в Firestore `users/{uid}` — чтобы ProfileCompletionBanner и серверные читатели увидели
  фамилию сразу, без ожидания следующего auth-sync.
- **Soft prompt**: `ProfileCompletionBanner` на `/booking/account` показывает inline-форму,
  если у юзера в `displayName` нет двух слов. Не блокирует страницу.
- **Хелперы**: `parseDisplayName` (split по последнему пробелу) и `hasFullName` —
  в [src/pages/booking/utils.ts](../../src/pages/booking/utils.ts).
- Существующие клиенты в alteg.io не синкаются автоматически — фамилия попадёт в alteg.io
  только при следующей брони (через `fullname` в `book_record`).
...
- ProfileCompletionBanner: для юзеров без фамилии — inline-форма "Укажите имя и фамилию"
...
## UX-изменения (Q1–Q2 2026)

### Сетка 15 минут
Шаг сетки изменён с 30 на 15 минут. `QUARTER_HOUR_PX = ROW_HEIGHT / 4`. Функции `yToQuarterSlot()` и `quarterSlotToTime()` конвертируют позицию курсора в 15-минутный слот.

### Mobile two-tap бронирование
На touch-устройствах (`isTouchDevice()`):
- Первый тап = выбор слота (цветная рамка + кнопка "Забронировать")
- Второй тап (или нажатие кнопки) = подтверждение брони
- На desktop поведение не изменилось (один клик = бронь)

### Тёмно-серые недоступные слоты
Hover на недоступном слоте: `rgba(120,120,120,0.15)` вместо красного. Менее агрессивный визуал.

### Валидация конца рабочего дня
`slotFits()` проверяет, что выбранная длительность не выходит за 22:00.

### Расширенный диапазон календаря
Навигация: текущая неделя + 8 недель вперёд (было +3). `MAX_WEEK_OFFSET = 8`.
