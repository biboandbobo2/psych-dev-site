...
| `findClients` | GET | Поиск клиентов по email И телефону |
...
2. Если нет — ищет в alteg.io по email И телефону (`findClients`)
3. Если не найден — создаёт клиента (`createClient`)
4. Сохраняет `altegClientIds[]` в Firestore
