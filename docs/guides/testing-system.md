...
### Жанры уровней (обязательны при создании цепочки по лекции)

Три уровня — это три **разных жанра** вопросов, не просто разные пороги сложности. Если все три уровня вырастают только из лекции, цепочка вырождается в один длинный тест.

**База (70%, palette `peach-sunset`)** — прямое содержание лекции.
- Что лектор называл, какие эксперименты разбирал, какие имена и формулировки звучали.
- Источник `resourcesRight[0]` — таймкод лекции; `[1]` — Wikipedia на термин или персону.

**Кейсы (75%, palette `lavender-calm`)** — перенос идей лекции на жизненные и клинические ситуации.
- Описание реального сценария + квалификация по понятиям лекции.
- Источники как в базе: таймкод фрагмента, где лектор разбирает близкий механизм + Wikipedia на конкретное понятие.

**Методология (80%, palette `fresh-mint`)** — **расширение и углубление тем, которые лектор упомянул, но подробно не разбирал**.
- Темы — те, на которые лектор сослалась вскользь, прямо отослала «у вас будет на клинике / на возрастной / почитайте», или контекст эпохи: методология науки, психометрика, классический эксперимент в полной форме, оригинальная позиция автора, история термина.
- **Запрещено**: тонкие различения позиций из самой лекции (это уже было в Базе и Кейсах). Если на вопрос можно ответить «послушав лекцию ещё раз внимательнее» — это не методология, а усложнённая база.
- Источник `resourcesRight[0]` — таймкод фрагмента, где тема впервые упомянута; `[1]` — Wikipedia или Stanford Encyclopedia **на конкретный термин**, не на общую статью лекции («Quasi-experiment», «Frederic_Bartlett», «Iconic_memory», «Гипотетико-дедуктивный_метод» — да; «Память — Википедия», «Мышление — Википедия» — нет).
- Хорошие источники тем: что лектор сама помечает фразами «подробнее на клинике», «это отдельная большая тема», «у Гальперина есть отдельная книга», «об этом писал Бартлетт».

Эталон — TEST3 в general-1, general-5, general-6: Хоторнский эффект, корреляция vs причинно-следственная связь, Self-report inventory, метод сбережения Эббингауза, метод частичного отчёта Сперлинга, центральный исполнитель Бэддели.

#### Дополнительные правила для TEST3

**Таймкод обязан попадать в содержательный фрагмент по теме вопроса.** Не годится таймкод, где лектор только проходом упоминает имя/тему («я уже говорила про X», «к нему ещё вернусь», «это отдельная тема»). Нужен фрагмент, где тема реально звучит. Если такого фрагмента в лекции нет — значит, тема НЕ упомянута и не подходит для TEST3 этой лекции. Перед публикацией каждый таймкод вручную проверять по транскрипту: открыть `[mm:ss]` в `lines.txt` и убедиться, что лектор говорит именно о теме вопроса.

**`resourcesRight[1]` должен вести на узкую тему вопроса, а не на общую персону или область.** Если вопрос про конкретную книгу Лурии 1962 года — ссылка не на «Лурия — Википедия», а на статью о книге, либо на конкретный раздел статьи об авторе через якорь (`#Основные_труды`). Если статьи на тему нет — Stanford Encyclopedia, открытая статья журнала, профильная страница университета. Общая «X — Википедия» в TEST3 запрещена, кроме случаев, когда сама статья посвящена ровно одной узкой теме.

**Дистракторы не должны выдавать ответ через простой формальный шаблон.** Запрещено: правильный — единственная советская книга, дистракторы — три западные; правильный — единственный термин из подобласти вопроса, остальные — из других подобластей. Дистракторы строить так, чтобы выбор шёл по знанию темы, а не по эпохе/языку/автору/принадлежности к области. Часто это означает поставить другие книги того же автора, другие субшкалы того же теста, другие эксперименты в той же подобласти.

**Explanation в TEST3 учит, а не перечисляет.** «Вариант B — Пиаже» — не разбор. Формула: «правильный — определение + пример или короткое уточнение. Дистрактор N описывает Y, к нашей теме не относится потому-то.» На каждый дистрактор — одна-две содержательные фразы.
...
npm run smoke:roles -- --with-functions          # + эмулятор Cloud Functions и сценарии functions, author-students
```
...
`?stream=`, плюс тумблер «лично» — он зовёт реальный `updateCourseAccess`,
поэтому `test.skip` без `--with-functions`), `coadmin.spec.ts` (страница и
вкладка «Потоки» открыты, секция «Права» в карточке заблокирована, карточка на
`/coadmin`) и `author.spec.ts` (админ курса получает «Access denied» от
`RequireCoAdmin`). Сценарии выдачи прав в `functions-admin.spec.ts` работают
через новый UI: окно «Добавить» → «Права администратора курса», в карточке —
...
За рамками стенда: внешние AI-эндпоинты (`/api/*`); прод-данные покрыты только контент-срезом `--prod-data` (пользовательские коллекции стенд не видит), деплой функций. Осталось из HP-2: e2e-job в CI.
...
### Группа D. Перед публикацией

#### 10. Самопроверка теста после создания

Перед `status: 'published'` — обязательный проход по мини-чек-листу. Большая часть наших правок в курсе общей психологии всплыла именно потому, что этот шаг был пропущен.

1. **Фактическая точность каждого вопроса.** Даты, авторство, события, формулировки концепций — сверь по двум источникам (например, Wikipedia + БРЭ или Stanford Encyclopedia). Особенно для тех, где «общеизвестно» — это ловушка (пример: основание MPI в Мюнхене Хекхаузеном — «не один, а вместе с Вайнертом»).
2. **Уникальность маркера.** Для каждого вопроса «угадай по X» — пробегись по остальным учёным из списка и спроси: может ли этот же маркер описать кого-то ещё? Если да — переформулируй или замени маркер.
3. **Единообразие формата ответов в группе.** Все имена в формате «Имя Фамилия» (а не вперемешку с инициалами); все варианты — одного грамматического рода; одинаковая пунктуация. Ошибка типа «один ответ без имени, остальные с именем» — это уже намёк студенту.
4. **Визуальная проверка каждой ссылки.** `curl 200` отсеивает только мёртвые. Открой URL в браузере, убедись, что страница про нужный термин, не paywall'енная, не сборник цитат, не общая монография. Если не подходит — заменяй до публикации.
5. **Чтение каждого `explanation` как студент.** Есть ли пример? Разобран ли каждый дистрактор? Если текст сводится к перечислению определений без разбора отличий — переписать по формуле из правила №6.
6. **Прогон 4 ответов глазами как «угадывающий».** Можешь ли ты выбрать правильный, **не зная содержания** — по длине, формату, гендерному маркеру, эпохе? Если да — переделай (правила 1–4).

После этой проверки публикуй. Если правки нужны после публикации — править через Admin SDK / Firestore прямой записью, не через UI (быстрее и без потери результатов студентов).
