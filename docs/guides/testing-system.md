...
**До написания цепочки — проверка лекции на достоверность.** Утверждения лектора, которые расходятся с научным консенсусом или подаются сильнее, чем позволяют данные, выписываются автору курса отдельным списком, и вопросов по этим фрагментам не делается. Пример из пилота «Введение» (2026-09): «бурный подростковый кризис — признак успешного предыдущего периода» и нормативность кризиса среднего возраста в тесты не вошли.
...
- В тексте вопроса не ссылаться на лектора («что бы ответила лекция», «лектор услышал бы») — это превращает кейс обратно в базу. Вопрос звучит прямо («Что происходит?»), лекция — только в `explanation` и ссылке.
...
- В курсе «Психология развития» этот уровень называется **«вглубь»** (например, «Введение — вглубь»): там он про темы за рамками лекции, а не про методы науки. В курсе общей психологии остаётся «методология».
...
**Порог уровня хранится в следующем тесте.** `requiredPercentage` теста — это порог по его prerequisite (кейсы 75% = «открываются при ≥75% за базу»). Поэтому зелёный бейдж на карточке и «Тест пройден!» на экране результатов считаются по `requiredPercentage` следующего уровня, а для последнего уровня — по собственному.

Как выставлять: чтобы база проходилась с 70%, а кейсы с 75%, ставим **кейсы `requiredPercentage: 70`**, **третий уровень `requiredPercentage: 75`**. У последнего уровня отдельного поля для собственного порога нет — он показывает свой же `requiredPercentage`. В пилотах dev-курса (2026-09) пороги сначала стояли со сдвигом на уровень (база фактически 75%, кейсы 80%) — исправлено.

**Порядок на `/tests-lesson`** — по порядку занятий курса (`courseNavIndex`, `sortTestsByLessonOrder`): легаси-рубрики сводятся к каноническим id, тесты с неизвестной рубрикой — в конце, внутри занятия сначала новые.

**Экран результатов** (`TestResultsScreen`) ищет следующий уровень (`getNextLevelTest`: опубликованный тест с `prerequisiteTestId` = текущий) и при пройденном пороге показывает кнопку «Следующий уровень →».
...
7. **Линтер JSON.** `npx tsx scripts/lintTestJson.ts <json>` ловит то, что глазами пропускается: кавычки или скобки только в части вариантов (цитата лектора в одном правильном ответе — прямая подсказка), правильный ответ заметно длиннее или короче остальных, правильный — самый длинный больше чем в 40% вопросов, категоричные слова («всегда», «только», «вообще», «невозможно»…) в двух и более неверных вариантах при их отсутствии в верном, буквенные ссылки на варианты в `explanation`, несовпадение таймкода в названии ссылки и `t=` в URL. `publishTestFromJson.ts` вызывает линтер сам и при ошибках не публикует.
8. **Чего линтер не видит — проверить глазами.** (а) Подсказка в тексте вопроса: вступление вроде «ребёнок уже живёт звуками маминой жизни» заранее называет ответ про голос матери. (б) «Золотая середина»: верный вариант стоит между двумя крайностями («полная свобода» и «строгая методика») и выбирается как самый разумный. (в) Нелепые неверные варианты («узнают голос отца лучше материнского») — их отбрасывают без знаний. Хорошие неверные варианты — реальные конкурирующие гипотезы и частые заблуждения из той же области.
...
- `resourcesWrong` = `resourcesRight`: иначе ошибившийся студент не видит ни таймкода, ни источника.
