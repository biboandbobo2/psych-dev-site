...
### MP‑6. Миграция данных Психологии развития на формат sections (P: M, E: M)
- **Проблема:** Курс "Психология развития" использует устаревший "плоский" формат данных (`video_url`, `concepts` array и т.д.), в то время как новые курсы (Clinical, General) используют структурированный объект `sections`.
- **Последствия:**
  - В коде приходится держать адаптеры (`convertLegacyToSections`, fallback-логика в `useContentLoader`).
  - Админка усложнена поддержкой двух форматов.
  - Риск рассинхронизации данных (как было с "залипанием" заглушек).
- **Цель:** Унифицировать формат данных для всех курсов, полностью перейдя на `sections`.
- **Задачи:**
  - [ ] Написать скрипт миграции (Node.js + Firebase Admin SDK), который пройдёт по всем документам в коллекции `periods` (и `intro`).
  - [ ] Скрипт должен сконвертировать плоские поля в объект `sections` (аналогично логике в `useContentSaver.ts`).
  - [ ] Скрипт должен удалить старые плоские поля после успешной конвертации.
  - [ ] После миграции: удалить функцию `convertLegacyToSections` из `PeriodPage.tsx`.
  - [ ] Упростить `useContentLoader.ts` и `useContentSaver.ts`, убрав fallback-логику для legacy полей.
  - [ ] Проверить работу всех страниц курса Психологии развития.
- **Файлы:** `src/pages/PeriodPage.tsx`, `src/pages/admin/content-editor/hooks/*`, скрипты миграции.
