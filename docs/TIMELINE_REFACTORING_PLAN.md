...
- Панели, формы и Canvas вынесены в отдельные компоненты; `Timeline.tsx` теперь только управляет данными и прокидывает цепочки пропсов.
- Утилиты `parseBulkEvents` и `formatEventAsNote` появились и покрыты Vitest (см. файлы в `src/pages/timeline/utils/__tests__`), так что Phase 3 (unit-тесты) частично стартовал.
- `BulkEventCreator` и `SaveEventAsNoteButton` используют новые утилиты, а сборка (`npm run build`) и тесты (`npm run test`) проходят без ошибок.
...
### Phase 6: Контроль качества и релиз

**Цель:** замкнуть рефакторинг проверками и документированными сценариями перед финальным релизом.

#### Задачи:
1. **Юнит-тесты:** покрыть интеграцию таймлайна и заметок (`SaveNoteAsEventButton`, `NoteModal`, `useTimeline`) плюс ключевые вспомогательные утилиты (`parseBulkEvents`, `formatEventAsNote` уже покрыты – нацелить вниманию на новые тесты из `src/components/__tests__/SaveNoteAsEventButton.test.tsx` и `src/hooks/__tests__/useTimeline.test.ts`).
2. **Manual smoke:** прогнать CRUD-заметки (создание, редактирование, удаление), экспорт заметок и создание события из заметки (типична ссылка на нужный поток).
3. **Релизная запись:** документировать результаты тестов/сборки, выверить `npm run build` и собрать чеклист smoke-тестов для финального PR/деплоя.

#### Контрольные точки:
- `SaveNoteAsEventButton` открывает модалку, валидация не пропускает ошибки, а `addEventToTimeline` вызывается с корректными данными.
- `NoteModal` работает с `useTimeline` без ошибок, сохраняет заметку и корректно сбрасывает состояние.
- `npm run test` и `npm run build` отрабатывают без ошибок – результаты фиксируются в документации и PR.

#### Результат Phase 6:
- Утверждённый чеклист ручных flows (CRUD, экспорт, событие).
- Покрито тестами критичное поведение интеграции заметок и таймлайна.
- Финальный коммит/PR сопровождается описанием всех фаз и ключевых тестов/сборок.
